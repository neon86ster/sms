...
if($status=="edit"&&$cs==false){
	...
	if($cs["book_type"]==2){
		$msg_booktype=" Member's Online Booking";
	}
...
?>
                  ...
                  <tr>
                    <td><?=$obj->makeListbox("tw[$i][$j][hour]","l_hour","hour_name","hour_id",$tw[$i][$j]["hour"],1,"hour_name",0,0,"hour_id>1 and hour_name >= \"00:15:00\"")?></td>
                  </tr>
             		
             	  <?
             	  	//Find End Book time
             	  	if(!$tw[$i]["tthour"]){
             	  		$tw[$i]["tthour"]=$cs["tthour"];
             	  	}	
             	  		$book_start_min = 60 * (8 + floor(($cs["appttime"]-1) / 12)) + 5 * (($cs["appttime"] -1) % 12);
                  		$hour_book = $obj->getIdToText($tw[$i]["tthour"],"l_hour","hour_name","hour_id");
                  ...
                  <tr>
                    <td>
                    <?=$obj->makeListbox("tw[$i][$j][start_id]","p_timer","time_start","time_id",$tw[$i][$j]["start_id"],1,0,0,0,"time_id>=".$cs["appttime"]." and time_id<=".$book_end[$i])?>
                    </td>
                  </tr>
                  ...
                  <?
                   ...
                   if(!$tw[$i][$j]["start_id"]){
                  	...
                  	//if(!$tw[$i][$j]["endtime"]){
                  		$time_start_min[$i][$j] = 60 * (8 + floor(($tw[$i][$j]["start_id"]-1) / 12)) + 5 * (($tw[$i][$j]["start_id"] -1) % 12);
                  		$hour_name = $obj->getIdToText($tw[$i][$j]["hour"],"l_hour","hour_name","hour_id");
                  		list ($hr, $min, $sec) = explode(":", $hour_name);
                  		$max_hour_min[$i][$j] = (60 * $hr) + $min;
                  		$time_end_min[$i][$j] = $time_start_min[$i][$j] + $max_hour_min[$i][$j];
                  	
                  		// convert time finish to id
						$time_end[$i][$j] = 12 * (floor( $time_end_min[$i][$j] / 60 ) - 8) + 1 + 
						( $time_end_min[$i][$j] - 60 * floor( $time_end_min[$i][$j] / 60 )) / 5; 
                  
                  		$tw[$i][$j]["endtime"]=$obj->getIdToText($time_end[$i][$j],"p_timer","time_start","time_id");
                  	//}
                  ?>
                  <tr>
                    <td>
                    <input style="background-color:#cccccc;" readonly="readonly" type="text" name='tw[<?=$i?>][<?=$j?>][endtime]' id='tw[<?=$i?>][<?=$j?>][endtime]' value="<?=$tw[$i][$j]["endtime"]?>" />
                    <input type="hidden" name='tw[<?=$i?>][<?=$j?>][end_id]' id='tw[<?=$i?>][<?=$j?>][end_id]' value="<?=$time_end[$i][$j]?>" />
                    </td>
                  </tr>
<? } ?>
