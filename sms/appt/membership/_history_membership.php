		...
		<td align="right" class="header" style="padding-right:12px;padding-bottom:0px;border-bottom: 2px solid #ffffff;">
			 <table border="0" height="20" cellpadding="0" cellspacing="0">
        	    <tr>
					...
					<? if($successmsg!=""){ ?><img src="/images/successmsg.png" /><? } ?>&nbsp;&nbsp;<b class="successmsg"><?=$successmsg?></b></span>&nbsp;&nbsp;
        	    	</td>
        	    	<td>
                        <span id="tabs">
                                <ul>
		...
		<td width="100%" style="padding-left: 5px"><br/>
			<div align="left" class="group5">
    			<fieldset>
					...
					<table class="generalinfo" cellpadding="0" cellspacing="0" style="margin: -15px;"> 
						<td valign="top" width="60%">
							   <table border="0" class="membership">
				        		...
				        		<tr height="20">
				        			...
				        			<!--		<input type="submit" name="balance" id="balance" value="<?=$balance?>" onClick="confirmBalance(<?="$membercode,$pageid"?>)"> -->
				        			</td>
				        		</tr>
			        			<?}else{?>
				        		<tr height="20">
...
<?
	...
	for ($i = 0; $i < $rssr["rows"]; $i++) {
		...
		$class = "class=\"odd\"";
		// amount calculater
		if ($rssr[$i]["pos_neg_value"] == 1) {
			$product["servicescharge"] = 0;
			$product["taxpercent"] = 0;

			if ($rssr[$i]["plus_servicecharge"]) {
				$product["servicescharge"] = $rssr[$i]["servicescharge"];
			}
			if ($rssr[$i]["plus_vat"]) {
				$product["taxpercent"] = $rssr[$i]["taxpercent"];
			}
...
?>					
				...
				?>
						<tr height="20px" <?=$class?>> 
								<td class="report"><?=($rssr[$i]["appt_date"]=="0000-00-00")?"-":$dateobj->convertdate($rssr[$i]["appt_date"],'Y-m-d',$sdateformat)?></td> 
								<td class="report"><?=$rssr[$i]["branch_name"]?></td>
								<td class="report"><?=$id?></td>
								<td class="report"><?=$rssr[$i]["pd_name"]?></td>
								...
								<td class="report" align="center" <?=$styleColor?>><?=$status?></td>
						</tr>
