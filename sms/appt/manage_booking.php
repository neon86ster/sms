...
if($status=="edit"&&$cs==false){
	...
	for($i=0;$i<$rs_tw["rows"];$i++){
		...
		$chksql = "select * from da_mult_th where indivi_id=".$tw[$i]["id"]." order by multh_id";
		...
		for($j=0;$j<$rs_multh["rows"];$j++){
			...
			$tw[$i][$j]["start_id"] = $rs_multh[$j]["start_id"];
			$tw[$i][$j]["end_id"] = $rs_multh[$j]["end_id"];
		}
...
if($status=="add"&&$cs){
	...
	$cs["cms_percent"] = $obj->getIdToText($cs["branch"],"bl_branchinfo","pcms_id","branch_id");
...
if($status=="edit"&&$cs==false){
	...
	$cs["cms_value"] = $rs_cs[0]["c_cms_value"];
	...
	$cs["book_type"] = $rs_cs[0]["book_type"];
	
	if($cs["book_type"]==2){
		$msg_booktype="Online Booking";
	}
...
$cs["cms_percent"] = (isset($cs["cms_percent"]))?$cs["cms_percent"]:$obj->getIdToText($cs["branch"],"bl_branchinfo","pcms_id","branch_id");
...
if($cs["memid"]){
$member_balance = $obj->checkMemberbalance($cs["memid"]);
$member_balance = number_format($member_balance,2,".","");
}
...
//======  End init first value  ================================================//
	for($j=0; $j<$srdcount[$i]; $j++){
			...
			if($obj->getIdToText($product["category_id"][$j],"cl_product_category","set_payment","pd_category_id")==0) {
				...
				//Get Total Member Takeout to check Member Balance
				if($cs["memid"]){
					if($obj->getIdToText($srd[$i][$j]["pd_id"],"cl_product","member_takeout","pd_id")==1 && $srd[$i][0]["paid"]!=1){
						if($obj->getIdToText($product["category_id"][$j],"cl_product_category","pos_neg_value","pd_category_id")==0) {
							$totaltakeout += $product["total"][$j];
							$totaltakeout += $obj->getsSvc($product,$j);
							$totaltakeout += $obj->getsTax($product,$j,$obj->getsSvc($product,$j));
						} else {
							$totaltakeout -= $product["total"][$j];
							$totaltakeout -= $obj->getsSvc($product,$j);
							$totaltakeout -= $obj->getsTax($product,$j,$obj->getsSvc($product,$j));
						}
						$totaltakeout = number_format($totaltakeout,2,".","");
						if($member_balance-$totaltakeout<0){
							$obj->setErrorMsg("Please check Membership Balance !!, Member Code : ".$cs["memid"]." has Balance ".$member_balance);
							$errormsg = $obj->getErrorMsg();
							$srd[$i][$j]["unit_price"]=$obj->getsNet($member_balance,$product,$j);
							$product["unit_price"][$j] = $srd[$i][$j]["unit_price"];
							$product["total"][$j] = $product["qty"][$j]*$product["unit_price"][$j];
						}
					}
...
if(!isset($gift_rows)){$gift_rows=0;}
if(!isset($gift[0]["number"])){$gift[0]["number"]=0;}
if(!isset($gift["gift_number"])){$gift["gift_number"]=0;}

$gift = $obj->getParameter("gift",0);
$gift_len = strlen($gift["gift_number"]);
$queueGift = $obj->getParameter("queueGift",0);
...
if(!isset($max_cstthour)){$max_cstthour="";}
// debugging all undified variable
$max_cstthour= (isset($max_cstthour))?$max_cstthour:"";
if(!isset($hourperiod[$max_cstthour])){$hourperiod[$max_cstthour]=0;}
if(!isset($hourperiod[$tw[$i]["tthour"]])){$hourperiod[$tw[$i]["tthour"]]=0;}
		if($hourperiod[$max_cstthour]<$hourperiod[$tw[$i]["tthour"]]){
			$max_cstthour=$tw[$i]["tthour"];
		}
	}
	$cs["tthour"]=$max_cstthour;
}
// #######################  Insert/Update Database #######################
if(isset($_POST["add"]) && !$newLogin && !$errormsg) {
	
//For Debug space before Insert/Update
	$cs["bpphone"]=str_replace(" ","",$cs["bpphone"]);
//
$cms_percent_number=$obj->getIdToText($cs["cms_percent"],"al_percent_cms","pcms_percent","pcms_id");
if(!$cs["cms_value"] || $cms_percent_number>0 || !$cs["cms"]){
	$cs["cms_value"]=0;
}
		if($status=="edit") {
			$errormsg = false;
...
<body style="background-color: #eae8e8;" onLoad="document.getElementById('loading').style.display='none';focusIt()">
<div id="loading" style="position:absolute;left:0px; top:0px; background-color: #FFFFFF;  width:100%; height:100%; text-align:center; opacity:0.70;display:block;z-index:1;">
     ...
     <? if($status=="add"){?>
      ...
      <table width="100%" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <b>Reservation 
            <? if($status=="edit"){?>
            		by: &nbsp;</b><b class="style1"><?=$obj->checkParameter($obj->getIdToText($cs["insertuser"],"s_user","u","u_id"),"- ")?><?=(isset($msg_booktype))?$msg_booktype:""?></b>
            ...
            <td align="right" style="border-bottom:2px solid #ffffff;"><span class="tabmenuheader" style="margin-right:20px">
             <? if($chkPageEdit){?>
                	...
                	<td align="center" bgcolor="#d6dff7" >
                        <span id="tabs">
                                <ul>
									...
									<li id="tabthree" <?=($chkpage==3)?"class=\"current\"":""?>><a href="javascript:;" onClick="chkPage(3);focusIt();"><span>Sales Receipt</span></a></li>
                                </ul>
                      </span>     
               	  </td>
                </tr>
                            ...
                            <tr>
                              ...
                              <td class="cc">
								...
								<?=$obj->makeListbox("cs[tthour$status]","l_hour","hour_name","hour_id",$cs["tthour"],0,"hour_name",0,0,"hour_id>1 and hour_name >= \"00:30:00\"")?>
              					<? }else{ ?>
								<?=$obj->makeListbox("clonecs_tthour","l_hour","hour_name","hour_id",$cs["tthour"],0,"hour_name",0,0,"hour_id>1 and hour_name >= \"00:30:00\"",0,1)?>
								<input type="hidden" id="cs[tthour]" name="cs[tthour]" value="<?=$cs["tthour"]?>"/>
								<? } ?>
                    ...
                    <td valign="bottom"><div>
                        <table cellspacing="0" cellpadding="0" width="97%" class="cusinfo">
                          ...
                          <tr>
                            ...
                            <?=$obj->makeListbox("cs[hotel]","al_accomodations","acc_name","acc_id",$cs["hotel"],false,"Replace(acc_name,' ','')","acc_active",1,"city_id=$cityid","acc_id=1",false,false,false,"this.className='ctrDropDown'")?>
                            </span>
                      ...
                      <td valign="bottom"><div>
						  ...
						  <table cellspacing="0" cellpadding="0" border="0" width="100%" class="cusinfo">
                            <tr>
                              <td>B.P. Name:</td>
                              <td class="cc">
							  <input disabled="disabled" type='text' size="22" value="<?=$cs["bpname"]?>" name="cs_lock[bpname]" id="cs_lock[bpname]" <?=$cmschk?>/>
							  <input type='hidden' size="22" value="<?=$cs["bpname"]?>" name="cs[bpname]" id="cs[bpname]" <?=$cmschk?>/>
							  </td>
                              <td colspan="2" class="cc">
                              &nbsp;&nbsp;&nbsp;B.P. PH #:&nbsp;&nbsp;
							  <input disabled="disabled" type='text' id='cs_lock[bpphone]' name='cs_lock[bpphone]' value="<?=$cs["bpphone"]?>" maxlength="15" style="width:75px;" onChange="checkBPphone(this,'<?=$bpphoneno?>')" <?=$cmschk?>/>
                              <input type='hidden' id='cs[bpphone]' name='cs[bpphone]' value="<?=$cs["bpphone"]?>" maxlength="15" style="width:75px;" onChange="checkBPphone(this,'<?=$bpphoneno?>')" <?=$cmschk?>/>
                            ...
                            <tr>
                              ...
                              <td class="ccinfo">
                               ...
                               <span style="width: 100px;font-family:Tahoma; font-size: 11px;overflow:hidden;">
                              <?=$obj->makeListbox("cs_lock[bcompany]","al_bookparty","bp_name","bp_id",$cs["bcompany"],0,"LOWER(bp_name)","bp_active","1",0,"bp_id=1",1,false,false,"this.className='ctrDropDown'")?>
                               <input type="hidden" id="cs[bcompany]" name="cs[bcompany]" value="<?=$cs["bcompany"]?>" >
							   </span>
                              ...
                              <td class="ccspecific" style="padding-bottom:10px;padding-top:5px;">
                               ...
                               <?=$obj->makeListbox("cs[inspection]","l_marketingcode,l_mkcode_category","sign","mkcode_id",$cs["inspection"],0,"LOWER(category_name),category_id,LOWER(sign)","active","1","l_marketingcode.category_id=l_mkcode_category.category_id",false,false,false,false,"this.className='ctrDropDown'")?> 
							   </span>&nbsp;
                            ...
                            <tr>
                              <td colspan="4" style="padding-left:0px;padding-right:0px;padding-top:10px;border-top:solid 1px #d0d0d0"><table cellspacing="0" cellpadding="0" border="0" width="100%">
                                  <tr>
                                    <td width="40%"><input disabled="disabled" type="checkbox" value="checked" id="cs[cms]" name="cs[cms]" onclick="showHideCheck('CV','cs[cms]');" class="checkbox" <?=$cs["cms"]?> <?=$cmschk?> />
                                      Commission</td>
                                    <td width="25%">Commission Percent: <span class="style1">*</span></td>
                                    <td width="26%">
									<?=$obj->makeListbox("cs_lock[cms_percent]","al_percent_cms","pcms_percent","pcms_id",$cs["cms_percent"],0,"pcms_percent","pcms_active","1",false,false,1)?>
									 <input type="hidden" id="cs[cms_percent]" name="cs[cms_percent]" value="<?=$cs["cms_percent"]?>" >
									</td>
                                  </tr>
                  
                                   <tr> 
                                   	<td colspan="3">
	                                	 <div id="CV" name="CV" <?=($cs["cms"])?"style=\"display:block\"":"style=\"display:none\""?>>  
		                                    <table width="100%" cellpadding="0" cellspacing="0">
		                                      <tr>
		                                        <td width="40%">&nbsp;</td>
			                                   	<td width="25%">Commission Value: <span class="style1">*</span></td>
												<?if(!isset($cs["cms_value"])){$cs["cms_value"]="";}?>
			                                    <td width="26%">
												<input disabled="disabled" type="text" value="<?=(is_numeric($cs["cms_value"]))?number_format($cs["cms_value"],2,".",""):number_format(0,2,".","")?>" id="cs_lock[cms_value]" name="cs_lock[cms_value]"/>
												<input type="hidden" value="<?=(is_numeric($cs["cms_value"]))?number_format($cs["cms_value"],2,".",""):number_format(0,2,".","")?>" id="cs[cms_value]" name="cs[cms_value]"/>
			                                  </tr>
		                                    </table>
	                                    </div>
                                	</td>
                          ...
                          <table cellspacing="0" cellpadding="0" border="0" width="100%" class="cusinfo">
                            ...
                            <tr>
                              ...
                              <td class="ccinfo">
                               ...
                               <span style="width: 100px;font-family:Tahoma; font-size: 11px;overflow:hidden;">
                              <?=$obj->makeListbox("cs[bcompany]","al_bookparty","bp_name","bp_id",$cs["bcompany"],0,"LOWER(bp_name)","bp_active","1",0,"bp_id=1",$cmschk,false,false,"this.className='ctrDropDown'")?>
                               </span>
                            ...
                            <tr>
                              <td colspan="4" style="padding-left:0px;padding-right:0px;padding-top:10px;border-top:solid 1px #d0d0d0"><table cellspacing="0" cellpadding="0" border="0" width="100%">
                                  <tr>
                                    <td width="40%"><input type="checkbox" value="checked" id="cs[cms]" name="cs[cms]" onclick="showHideCheck('CV','cs[cms]');" class="checkbox" <?=$cs["cms"]?> <?=$cmschk?> />
                                      Commission</td>
                                    <td width="25%">Commission Percent: <span class="style1">*</span></td>
		                                    ...
		                                    <table width="100%" cellpadding="0" cellspacing="0">
		                                      <tr>
		                                        <td width="40%">&nbsp;</td>
			                                   	<td width="25%">Commission Value: <span class="style1">*</span></td>
												<?if(!isset($cs["cms_value"])){$cs["cms_value"]="";}?>
			                                    <td width="26%"><input type="text" value="<?=(is_numeric($cs["cms_value"]))?number_format($cs["cms_value"],2,".",""):number_format(0,2,".","")?>" id="cs[cms_value]" name="cs[cms_value]"/>
			                                  </tr>
		                                    </table>
	                                    </div>
                                	</td>
                        ...
                        <td valign="bottom"><div>
                            <table cellspacing="0" width="100%" cellpadding="0" class="cusinfo">
                              <tr>
                                ...
                                <?if($cs["book_type"]==2){?>
								<td width="27%"><?=$obj->makeListbox("cs_lock[rs]","l_employee","emp_nickname","emp_id",$cs["rs"],0,"emp_nickname","emp_active","1","emp_department_id in (3,5)","emp_id=1",1)?></td>
								<input type="hidden" id="cs[rs]" name="cs[rs]" value="<?=$cs["rs"]?>">
								<?}else{?>
								<td width="27%"><?=$obj->makeListbox("cs[rs]","l_employee","emp_nickname","emp_id",$cs["rs"],0,"emp_nickname","emp_active","1","emp_department_id in (3,5)","emp_id=1")?></td>
                                <?}?>
...
?>
              <td><table width="100%" border="0" cellspacing="0" class="cusinfo" cellpadding="0">
                  ...
                  <tr>
                    <td><?=$obj->makeListbox("tw[$i][tthour]","l_hour","hour_name","hour_id",$tw[$i]["tthour"],1,"hour_name",0,0,"hour_id>1 and hour_name >= \"00:30:00\"")?></td>
                  </tr>
                  <tr>
                    <td>&nbsp;</td>
                  </tr>
...
?>
                  <tr>
                    <td class="ccinfo" style="padding-left:5">&nbsp;
                        ...
                        <?=$obj->makeTherapistlistBook("tw[$i][$j][name]",$tw[$i][$j]["name"],0,"l_employee.branch_id,l_employee.emp_nickname,l_employee.emp_code","bl_branchinfo.city_id=$cityid")?>
                  ...
                  <tr>
             	  ...
             	  <?
             	  	//Find End Book time
             	  		...
             	  		$book_start_min = 60 * (8 + floor(($cs["appttime"]-1) / 12)) + 5 * (($cs["appttime"] -1) % 12);
                  		$hour_book = $obj->getIdToText($cs["tthour"],"l_hour","hour_name","hour_id");
                  		list ($hr, $min, $sec) = explode(":", $hour_book);
                  		$max_book_min[$i] = (60 * $hr) + $min;
                  		
                  		$book_end_min[$i] = $book_start_min + $max_book_min[$i];
                  	
                  		// convert time finish to id
						$book_end[$i] = 12 * (floor( $book_end_min[$i] / 60 ) - 8) + 1 + 
						( $book_end_min[$i] - 60 * floor( $book_end_min[$i] / 60 )) / 5;
             	  
             	  		$book_end_name=$obj->getIdToText($book_end[$i],"p_timer","time_start","time_id");
      				//
      				
      				if(!$tw[$i][$j]["thid"]){
						$tw[$i][$j]["start_id"]=$cs["appttime"];
      					$book_start_min = 60 * (8 + floor(($tw[$i][$j]["start_id"]-1) / 12)) + 5 * (($tw[$i][$j]["start_id"] -1) % 12);
                  		$hour_book = $obj->getIdToText($tw[$i][$j]["hour"],"l_hour","hour_name","hour_id");
                  		list ($hr, $min, $sec) = explode(":", $hour_book);
                  		$max_book_min[$i] = (60 * $hr) + $min;
                  		...
                  		// convert time finish to id
						$tw[$i][$j]["end_id"] = 12 * (floor( $book_end_min[$i] / 60 ) - 8) + 1 + 
						( $book_end_min[$i] - 60 * floor( $book_end_min[$i] / 60 )) / 5;
      				}
             	  ?>
                  ...
                  <tr>
                    <td>
                    <?=$obj->makeTimeListbox("tw[$i][$j][start_id]","p_timer","time_start","time_id",$tw[$i][$j]["start_id"],0,0,0,0,/*"time_id>=".$cs["appttime"]." and time_id<=".$book_end[$i]*/0,0,0,0,0,"checkTime($i,$j);")?>
                    -&nbsp;<?=$obj->makeTimeListbox("tw[$i][$j][end_id]","p_timer","time_start","time_id",$tw[$i][$j]["end_id"],0,0,0,0,/*"time_id>=".$cs["appttime"]*/0,0,0,0,0,"checkTime($i,$j);")?>
                    </td>
                  </tr>
                  
                  <?
                  
                  	$time_name[$i][$j]=$obj->getIdToText($tw[$i][$j]["start_id"],"p_timer","time_start","time_id");		
                  	
                   if(!$tw[$i][$j]["start_id"]){
                   	$tw[$i][$j]["start_id"]=$cs["appttime"];
                   }else if($time_name[$i][$j]>$book_end_name){
                  		$tw[$i][$j]["start_id"]=$cs["appttime"];
                   }else{
                   		$tw[$i][$j]["start_id"]=$tw[$i][$j]["start_id"];
                   }
...
<? } ?>
                  ...
                  <? if($status=="edit" or $status=="add"){?>
                  <tr>
                    <td align="center"><a href="javascript:;;" onClick="if(check_csivalue(<?=$tw[$i]["id"]?>)){miniwindow('csi.php?book_id=<?=$bookid?>&indivi_id=<?=$tw[$i]["id"]?>&branch_id=<?=$cs["branch"]?>','csi','700','600','1','13','20','0')}" class="worksheet">&nbsp;CSI</a></td>
                  </tr>
                  <? }?>
              </table></td>
              <? 		}
            ...
            <tr >
             <td style=" vertical-align:top" width="35%">
                  ...
                  $sub=0;
                    ...
                    <tr>
                    	<td>
						<div style="overflow:auto;border:1px solid #eaeaea;<?if($gift_rows>0){?>height:130px;<?}?>">
						<? for($i=0;$i<$gift_rows;$i++){
						if($i%2==1){
...
$class =  "class=\"even\"  onmouseover=\"this.style.backgroundColor='#b0dfde'\" style=\"margin:0px; background-color:#eaeaea\" onmouseout=\"this.style.backgroundColor='#eaeaea'\"";
					...
					<table border="0" cellpadding="0" cellspacing="0" width="100%" bgcolor="#d3d3d3"class="<?=$class?>" style="border:1px solid #d3d3d3;">
	                        <tr style="">
		                    <td width="2%" > <input type="checkbox" name="giftnumber[]" id="giftnumber[]" value="<?=$gift[$i]["id"]?>" style="width:20px;" /></td>
		                    ...
		                    <input type="hidden" id="gift[<?=$i?>][product]" name="gift[<?=$i?>][product]" value="<?=$gift[$i]["product"]?>"/>
							  ...
							  <td width="51%"  align="left" valign="bottom" ><b style="font-size:10px;">Gift Number : <?=$gift[$i]["number"]?></b><br />
							    Gift To : <?=$gift[$i]["to"]?>
						        <br />
						        Value : 
					          <?=$gift[$i]["value"]?></td>
...
?>
        <td><table cellspacing="0" cellpadding="0" width="275px" class="cusinfo">
			<tr>
				<td>
				<input type="text" class="barcode" id="barcode[<?=$i?>]" name="barcode[<?=$i?>]" value="" onKeypress="checkKey(this,'appt','<?=$i?>',event,'<?=$cs["branch"]?>')">
            ...
            <tr>
              ...
              <?=$obj->makeListbox("srd[$i][$j][pd_id]","cl_product_category,cl_product","pd_name","pd_id",$srd[$i][$j]["pd_id"],$chkautosubmit,"cl_product_category.pd_category_priority, cl_product.pd_name","cl_product.pd_active",1,"cl_product.pd_category_id=cl_product_category.pd_category_id",false,!$chkSR)?>
                ...
                /
                <input size="1" id= "srd[<?=$i?>][<?=$j?>][quantity]" name="srd[<?=$i?>][<?=$j?>][quantity]" value="<?=($srd[$i][$j]["quantity"])?$srd[$i][$j]["quantity"]:"1"?>" maxlength="10" type="text" style="width:20px" <?=($chkSR)?"":"disabled"?> onChange="checkqtyNum(this);this.form.submit();"/>
                /
          ...
          for($j=0; $j<$mpdcount[$i]; $j++){
    		...
    		?>
             ...
             if(!$srd[$i][$j]["pay_price"]){
             	...
             	if($mpdcount[$i]==1){
             	echo number_format($r_total[$i],2,".","");
             	}else{
