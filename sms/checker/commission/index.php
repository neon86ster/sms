...
$bank = $obj->getParameter("bank_id",false);
...
$sortby = $obj->getParameter("chksortby","A &gt Z");
$bank_acc = $obj->getParameter("bank_acc","All");

$chkddreport = $obj->getParameter("chkddreport",0);
if($chkddreport){
	$ddreport = "All Report";
}else{
	$ddreport = "DD Report";
...
$querystr = "pageid=$pageid&begin=$hidden_begin&end=$hidden_end&branchid=$branch&order=$order&sortby=$sortby&ddreport=$ddreport&chkddreport=$chkddreport&bank=$bank&bank_acc=$bank_acc";
$print = "report.php?$querystr&export=print&ddreport=$ddreport&chkddreport=$chkddreport&bank=$bank&bank_acc=$bank_acc";
//$querystr = "pageid=$pageid&begin=$hidden_begin&end=$hidden_end&branchid=$branch&order=$order&sortby=$sortby&ddreport='+document.getElementById('chkddreport1').value+'";
//$print = "report.php?$querystr&export=print&ddreport='+document.getElementById('chkddreport1').value+'";
?>
<html>
<head>
...
<form id="cms" name="cms" action='' method='post' style="padding:0;margin:0">
			       ...
			       To: <input id='end' name='end' value="<?=$end?>" readonly="1" class="textbox" type="text" style="width:85px"/>
			        ...
			        <div id="date_end" class="dateChooser" style="display: none; visibility: hidden;background: #aea; padding-top: 5px; padding: 5 0 0 0;" align="center"></div>
				        ...
				        <select id="order" name="order">
				          ...
				          <option title="Phone" value="Phone Number" <?=($order=="Phone Number")?"selected":""?>>Phone Number</option>
				          <option title="Branch" value="Branch" <?=($order=="Branch")?"selected":""?>>Branch</option>
				         </select>
						...
						<?}else{?>
						Bank:
						<?=$obj->makeListbox("bank_id","all_l_bankname","bank_Ename","bank_id",$bank,0,"bank_Ename","bank_active","1","")?>
						<?}?>
						Branch:
    		...
    		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			      <tr>
        			<td height="30" class="rheader" style="padding-left: 20px;background-image: url('/images/<?=$theme?>/appt/menubg.png');"> Export:&nbsp;&nbsp;
			          ...
			          <!--&nbsp;&nbsp;<input type="button" name="ddreport" id="ddreport" value="DD Report" onClick="chkreport('<?=$querystr?>');"/>-->
					  &nbsp;&nbsp;<input type="submit" name="ddreport" id="ddreport" value="<?=$ddreport?>" onClick="changeDDreport(this)"/>
			          <input type="hidden" name="chkddreport" id="chkddreport" value="<?=$chkddreport?>"/>
					  <!--<input type="hidden" id="chkddreport" value="1"/><input type="hidden" id="chkddreport1" value="0"/>-->
					 ...
					 <?
						...
						?>
						Bank Account:
				        <select id="bank_acc" name="bank_acc">
						  <option title="All" value="All" <?=($bank_acc=="All")?"selected":""?>>All</option>
				          <option title="Yes" value="Yes" <?=($bank_acc=="Yes")?"selected":""?>>Yes</option>
				          <option title="No" value="No" <?=($bank_acc=="No")?"selected":""?>>No</option>
				         </select>
					 <?}?>
