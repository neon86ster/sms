...
$rs = $obj->getthpl(0, $begin_date, $end_date);
...
if ($export == "Excel" && $chkPageView) {
	...
	header("Content-Disposition: attachment; filename=\"Therapist Hours Per Location.xls\"");
				...
				for ($k = 0; $k < $rsbranch["rows"]; $k++) {
					if ($rsbranch[$k]["branch_category_id"] == $rsbranchtype[$j]["branch_category_id"] && $rsbranch[$k]["city_id"] == $rscity[$i]["city_id"] && $rsbranch[$k]["branch_active"] == 1) {
						...
						for ($d = 0; $d < $rsdate["rows"]; $d++) { // start date total loop
							...
							$total[$k] += $tmp[$k][$d];
							$alldatetotal[$d]=(isset($alldatetotal[$d]))?$alldatetotal[$d]:0;
						}
					}
				}
			}
...
if ($export != "Excel" && $export) {
	...
	if ($collapse == "Collapse") {
		$chkrow = $obj->getParameter("chkrow", 27);
	} else {
...
$reportname = "Therapist Hours Per Location";
...
?>		
				<td align="right" style="border-top:1px #000000 solid;">
						<? list($hr,$min) = explode(".",number_format($bttotal[$i][$j][$d],2,".",",")); ?>
						<?=$hr.".".$minsperiod[$min]?>
				</td>
				<? }  ?>	
				<? if($column!="Total only"&&$a==$alltable-1){ ?>
				<td align="right" style="border-top:1px #000000 solid;">
						<? list($hr,$min) = explode(".",number_format($allbttotal[$i][$j],2,".",",")); ?>
						<?=$hr.".".$minsperiod[$min]?>
				</td>
				<? } ?>
				<? if($percent&&$a==$alltable-1){ ?>
...
?>
		...
		<tr height="35">
			...
			<td align="right">
					<? list($hr,$min) = explode(".",number_format($allltotal[$i],2,".",",")); ?>
					<b><?=$hr.".".$minsperiod[$min]?></b>
			</td>
			<? if($percent){ ?>
				<td align="right">
...
<?
										...
										if ($order != "Total") // for colapse information report
											{
    	...
    	<td class="content" width="100%" align="center">
			...
			<tr>
				...
				<tr height="32">
					...
					<? }?>
					<? if($percent){?>
						<td style="text-align:center;padding-left:10px;overflow:hidden; border-top:2px #000000 solid;border-bottom:2px #ff0000 solid;"><b style="text-decoration: underline;">PERCENT</b></td>
					<? }?>
				</tr>
<?
...
?>
						<tr height="22">
							...
							<? for($d=0;$d<$rsdate["rows"];$d++){ ?>		
								<td align="right">
									<?if($export==false){?><a class="pagelink" href="javascript:;" style="text-decoration:none; color:#000000;" onClick="opentthourDetail(<?=$rsdate["begin"][$d].",".$rsdate["end"][$d].",".$rsbranch[$k]["branch_id"].",".$rscity[$i]["city_id"].",".$rsbranchtype[$j]["branch_category_id"]?>)"><?}?>	
											<? list($hr,$min) = explode(".",number_format($tmp[$k][$d],2,".",",")); ?>
											<?=$hr.".".$minsperiod[$min]?>
									<?if($export==false){?></a><?}?>
								</td>
							...
							<? if($column!="Total only"){ ?>
							<td align="right">
							<?if($export==false){?><a class="pagelink" href="javascript:;" style="text-decoration:none;color:#000000;" onClick="opentthourDetail(<?=$rsdate["begin"][0].",".$rsdate["end"][$d-1].",".$rsbranch[$k]["branch_id"].",".$rscity[$i]["city_id"].",".$rsbranchtype[$j]["branch_category_id"]?>)"><?}?>
									<? list($hr,$min) = explode(".",number_format($total[$k],2,".",",")); ?>
									<?=$hr.".".$minsperiod[$min]?>
							<?if($export==false){?></a><?}?>
							</td>
...
?>
				...
				<td align="right" style="border-top:1px #000000 solid;">
				<?if($export==false){?><a class="pagelink" href="javascript:;" style="text-decoration:none; color:#000000;" onClick="opentthourDetail(<?=$rsdate["begin"][$d].",".$rsdate["end"][$d].",0,".$rscity[$i]["city_id"].",".$rsbranchtype[$j]["branch_category_id"]?>)"><?}?>
							<? list($hr,$min) = explode(".",number_format($bttotal[$i][$j][$d],2,".",",")); ?>
							<?=$hr.".".$minsperiod[$min]?>
				<?if($export==false){?></a><?}?>
				</td>
				...
				<? if($column!="Total only"){ ?>
				<td align="right" style="border-top:1px #000000 solid;">
				<?if($export==false){?><a class="pagelink" href="javascript:;" style="text-decoration:none; color:#000000;" onClick="opentthourDetail(<?=$rsdate["begin"][0].",".$rsdate["end"][$d-1].",0,".$rscity[$i]["city_id"].",".$rsbranchtype[$j]["branch_category_id"]?>)"><?}?>
							<? list($hr,$min) = explode(".",number_format($allbttotal[$i][$j],2,".",",")); ?>
							<?=$hr.".".$minsperiod[$min]?>
				<?if($export==false){?></a><?}?>
				</td>
...
?>
		<tr height="35">
			...
			<td align="right">
			<?if($export==false){?><a class="pagelink" href="javascript:;" style="text-decoration:none; color:#000000;" onClick="opentthourDetail(<?=$rsdate["begin"][0].",".$rsdate["end"][$d-1].",0,".$rscity[$i]["city_id"].",0"?>)"><?}?>
						<? list($hr,$min) = explode(".",number_format($allltotal[$i],2,".",",")); ?>
						<b><?=$hr.".".$minsperiod[$min]?></b>
			<?if($export==false){?></a><?}?>
			</td>
...
?>		
			<td align="right">
			<?if($export==false){?><a class="pagelink" href="javascript:;" style="text-decoration:none; color:#000000;" onClick="opentthourDetail(<?=$rsdate["begin"][$d].",".$rsdate["end"][$d].",".$rsbranch[$k]["branch_id"].",0,0"?>)"><?}?>
						<? list($hr,$min) = explode(".",number_format($tmp[$k][$d],2,".",",")); ?>
						<?=$hr.".".$minsperiod[$min]?>
			<?if($export==false){?></a><?}?>
			</td>
...
?>
			<? if($column!="Total only"){ ?>
			<td align="right">
			<?if($export==false){?><a class="pagelink" href="javascript:;" style="text-decoration:none; color:#000000;" onClick="opentthourDetail(<?=$rsdate["begin"][0].",".$rsdate["end"][$d-1].",".$rsbranch[$k]["branch_id"].",0,0"?>)"><?}?>
						<? list($hr,$min) = explode(".",number_format($total[$k],2,".",",")); ?>
						<?=$hr.".".$minsperiod[$min]?>
			<?if($export==false){?></a><?}?>
			</td>
...
?>		
			<td align="right">
			<?if($export==false){?><a class="pagelink" href="javascript:;" style="text-decoration:none; color:#000000;" onClick="opentthourDetail(<?=$rsdate["begin"][$d].",".$rsdate["end"][$d].",".$tmpbranchtotal[$k].",0,0"?>)"><?}?>
			<?
...
?>
			<? if($column!="Total only"){ ?>
			<td align="right">
			<?if($export==false){?><a class="pagelink" href="javascript:;" style="text-decoration:none; color:#000000;" onClick="opentthourDetail(<?=$rsdate["begin"][0].",".$rsdate["end"][$d-1].",".$tmpbranchtotal[$k].",0,0"?>)"><?}?>
						<? list($hr,$min) = explode(".",number_format($total[$k],2,".",",")); ?>
						<?=$hr.".".$minsperiod[$min]?>
			<?if($export==false){?></a><?}?>
			</td>
...
<?
										...
										for ($d = 0; $d < $rsdate["rows"]; $d++) { // start branch total loop
											list($hr,$min) = explode(".",number_format($alldatetotal[$d],2,".",","));
											$allindate = $hr.".".$minsperiod[$min];
											echo "<td style=\"padding-left: 20px; white-space: nowrap;\" align=\"right\" bgcolor=\"#d3d3d3\">";
											if ($export == false) {
												echo "<a class=\"pagelink\" href=\"javascript:;\" style=\"text-decoration:none; color:#000000;\" onClick=\"opentthourDetail(" . $rsdate["begin"][$d] . "," . $rsdate["end"][$d] . ",0,0,0)\"><b>$allindate</b></a></td>";
											} else {
												echo "<b>$allindate</b></td>";
											}
										}
?>
			<? if($column!="Total only"){ ?>
			<td align="right" bgcolor="#d3d3d3">
			<?if($export==false){?><a class="pagelink" href="javascript:;" style="text-decoration:none; color:#000000;" onClick="opentthourDetail(<?=$rsdate["begin"][0].",".$rsdate["end"][$d-1].",0,0,0"?>)"><?}?>
						<? list($hr,$min) = explode(".",number_format($alltotal,2,".",",")); ?>
						<b><?=$hr.".".$minsperiod[$min]?></b>
...
<?
										...
										?>
													<? if($column!="Total only"){ ?>
													<td align="right" bgcolor="#d3d3d3"><b>
													<?if($export==false){?><a class="pagelink" href="javascript:;" style="text-decoration:none;" onClick="opentthourDetail(<?=$rsdate["begin"][0].",".$rsdate["end"][$d-1].",0,0,0"?>)"><?}?>
													<?=number_format(100*$alltotal/$palltotal,2,".",",")?>
