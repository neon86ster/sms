...
$querystr = "pageid=$pageid&begin=$hidden_begin&end=$hidden_end&date=$date&column=$column&order=$order&Collapse=$collapse&sortby=$sortby";
...
<tr>
 ...
 <form name="cpl" id="cpl" action="index.php" method="post" style="padding:0;margin:0">
    		...
    		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			      <tr>
        			<td height="30" class="rheader" style="padding-left: 20px;background-image: url('/images/<?=$theme?>/appt/menubg.png');"> Export:&nbsp;&nbsp;
			          ...
			          <!--
			          &nbsp;&nbsp;<input type="button" name="Export" id="Export" value="Export" onClick="window.open('report.php?begin=<?=$hidden_begin?>&end=<?=$hidden_end?>&date=<?=$date?>&column=<?=$column?>&order=<?=$order?>&Collapse=<?=$collapse?>&percent=<$percentchk?>&sortby=<?=$sortby?>&export='+document.getElementById('export').value)"/>
			          // For debug undefined variable : percentchk. Remove percentchk because not use and not initial in any where.
			          // By Ruck : 21-05-2009
			          -->
			          ...
			          <!--
			          &nbsp;&nbsp;<input type="checkbox" name="percent" id="percent" value="checked" <$percentchk?>/> 
			          &nbsp;&nbsp;percent
			          -->
           			</td>
			      </tr>
