...
$order=$obj->getParameter("order","client_name");
...
//query language
$sql = "select * from p_clientinfo ";
$search = strtolower($search);
// specific $search don't care about category select
$sql .= "where (lower(client_name) like '%$search%' " .
		"or lower(client_url) like '%$search%' " .
		"or lower(database_name) like '%$search%' ) ";
	...
	for($j=0;$j<$column;$j++){
		if($arrShowinList[$j]!="no") {
				...
				if($arrFields[$j]=='active') {
					if($chkPageEdit){
						if($rs[$i]["active"]==1){
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('p_clientinfo',".$rs[$i]["client_id"].",0);\"\>".
								 	"<img src=\"/images/active.png\" border=\"0\" title=\"active\" /></a>";
						}else{
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('p_clientinfo',".$rs[$i]["client_id"].",1);\">".
				...
				}else if($arrFields[$j]=='client_name'||$arrFields[$j]=='client_url'||$arrFields[$j]=='database_name'){
...
<?		}
	...
	if($chkPageEdit){
		echo "<td class=\"report\"><a href=\"javascript:;\" onclick=\"editData('$tbname','".$rs[$i]["client_id"]."')\">update</a></td>";
	}
	echo "</tr>";
}
if(!$rs["rows"]){}
