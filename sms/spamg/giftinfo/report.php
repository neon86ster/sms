...
$sql .= "where lower(gift_number) like '%$search%' " .
	"or lower(give_to) like '%$search%' " .
	"or lower(receive_from) like '%$search%' " .
	"or lower(product) like '%$search%' " .
	"or lower(value) like '%$search%' ";
	...
	for($j=0;$j<$column;$j++){
		if($arrShowinList[$j]!="no") {
				...
				if($arrFields[$j]=='book_id') {
					...
					if($chkPageEdit){
						if($rs[$i]["available"]==1){
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('g_gift',".$rs[$i]["gift_id"].",0);\"\>".
								    "<img src=\"/images/active.png\" border=\"0\" title=\"active\" /></a>";
						}else{
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('g_gift',".$rs[$i]["gift_id"].",1);\">".
...
<?		}
	...
	if($chkPageEdit){
		echo "<td class=\"report\"><a href=\"javascript:;\" onclick=\"editData('$tbname','".$rs[$i]["gift_id"]."')\">update</a></td>";
	}
	echo "</tr>";
}
if(!$rs["rows"]){}
