...
if($obj->getParameter("date")==2){
	$hidden_begin = $_POST["hidden_begin"];
	$hidden_end = $_POST["hidden_end"];
}else{
...
$selectDay="";
$year = substr($hidden_begin,0,4);
$month = substr($hidden_begin,4,2);
$day = substr($hidden_begin,6,2);
$punix_time = mktime(0, 0, 0, (int)$month, (int)$day-1, (int)$year);
$year = substr($hidden_end,0,4);
$month = substr($hidden_end,4,2);
$day = substr($hidden_end,6,2);
$nunix_time = mktime(0, 0, 0, (int)$month, (int)$day+1, (int)$year);
$selectDay = $obj->getParameter("selectDay");
if($selectDay==" Next Day "){
	$date = 2;
	$begin = date($sdateformat, $nunix_time);
	$hidden_begin = date("Ymd", $nunix_time);
	$end = $begin;
	$hidden_end = $hidden_begin;
} else if($selectDay==" Previous Day "){
	$date = 2;
	$end = date($sdateformat, $punix_time);
	$hidden_end = date("Ymd", $punix_time);
	$begin = $end;
	$hidden_begin = $hidden_end;
}

$querystr = "pageid=$pageid&begin=$hidden_begin&end=$hidden_end&branchid=$branch&cityid=$city";
...
<form name="trans" id="trans" action="" method="post" style="padding:0;margin:0">
	  ...
	  <tr>
	    <td valign="top" align="center" height="10">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
			        <td class="rheader" style="padding-left: 20px; width:170px;background-image: url('/images/<?=$theme?>/appt/menubg.png');">
			        ...
			        <img align="top" style="margin-top: 3px;" src="/images/calendar.png" alt="Date Appointment" onClick="showChooser(this, 'begin', 'date_begin', 1900, 2100, '<?=$sdateformat?>', false,false);" />
			        ...
			        <img align="top" style="margin-top: 3px;" src="/images/calendar.png" alt="Date Appointment" onClick="showChooser(this, 'end', 'date_end', 1900, 2100, '<?=$sdateformat?>', false,false);" />
			        ...
			        <input type="submit" name="selectDay" value=" Previous Day " title="<?=date($sdateformat, $punix_time)?>" >
					<input type="submit" name="selectDay" value=" Next Day " title="<?=date($sdateformat, $nunix_time)?>">
		        	</td>
