...
function editot(th,ot,setactive){
	
	
	var querystr = "&th="+th+"&ot="+ot+"&setactive="+setactive;
	var pageid = window.top.document.getElementById("leftFrame").contentWindow.document.getElementById("pageid").value;
	querystr = querystr+"&pageid="+pageid;
	gotoURL("index.php?"+querystr);
...
function sigIn(){
	...
	if(document.getElementById('thid').value!=1){
		if(document.getElementById('not').checked){
		window.open ('addinfo.php?thid='+ (document.getElementById('thid').value)
					+'&not='+ (document.getElementById('not').value)+'', 'therapist','location=1,top=200,left=500,scrollbars=0, width=250,height=250');
		}else{
		window.open ('addinfo.php?thid='+ (document.getElementById('thid').value)
					+'&not=0', 'therapist','location=1,top=200,left=500,scrollbars=0, width=250,height=250');
		}
