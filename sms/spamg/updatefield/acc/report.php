...
if($export=="Excel" && $chkPageView){
	...
	header("Content-Disposition: attachment; filename=\"Accommodation Information.xls\"");
...
//query language
$sql = "select * from al_accomodations ";
$search = strtolower($search);
// specific $search don't care about category select
if(!$showinactive){
$sql .= "where acc_active=1 ";
if($categoryid>0){$sql .= "and city_id=$categoryid ";}
$sql .= "and (lower(acc_name) like '%$search%' "  .
		"or lower(REPLACE(acc_detail,\"[br]\",\"\")) like '%$search%') ";
}else{
	if($categoryid>0){
		$sql .= "where city_id=$categoryid " .
			"and (lower(acc_name) like '%$search%' "  .
			"or lower(REPLACE(acc_detail,\"[br]\",\"\")) like '%$search%') ";
	}else{
		$sql .= "where lower(acc_name) like '%$search%' "  .
		"or lower(REPLACE(acc_detail,\"[br]\",\"\")) like '%$search%' ";
	}
}
if($cmsid){$sql .= "and acc_pcms=$cmsid ";}
...
for($i=0;$i<$rs["rows"];$i++){
	...
	for($j=0;$j<$column;$j++){
		if($arrShowinList[$j]!="no") {
				...
				}else if ($arrFields[$j]=='acc_country'){
					$data=$obj->getIdToText($rs[$i]['acc_country'],"dl_nationality","nationality_name","nationality_id");
				}else if ($arrFields[$j]=='acc_pcms'){
					$data=$obj->getIdToText($rs[$i]['acc_pcms'],"al_percent_cms","pcms_percent","pcms_id");
				}else if($arrFields[$j]=='acc_active') {
					if($chkPageEdit){
						if($rs[$i]["acc_active"]==1){
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('al_accomodations',".$rs[$i]["acc_id"].",0);\"\>".
								 "<img src=\"/images/active.png\" border=\"0\" title=\"active\" /></a>";
						}else{
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('al_accomodations',".$rs[$i]["acc_id"].",1);\">".
				...
				}else if($arrFields[$j]=='acc_detail'){
...
<?		}
	...
	if($chkPageEdit && !$export){
		echo "<td class=\"report\"><a href=\"javascript:;\" onclick=\"editData('$tbname','".$rs[$i]["acc_id"]."')\">update</a></td>";
	}
	echo "</tr>";
}
if(!$rs["rows"]){}
