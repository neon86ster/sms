...
$querystr = "&pageid=$pageid&search=$searchstr&order=$order&page=$page&sort=$sort&showinactive=$showinactive&cmsid=$cmsid&categoryid=".$categoryid;
//set room active
    	...
    	<td width="100%">
        	...
        	<div id="gform">
        		        <? 
							if($accid){
									$xml = "<command>" .
									"<table>al_accomodations</table>" .
									"<where name='acc_id' operator='='>$accid</where>" .
									"</command>";
								echo $obj->gFormEdit($xml,$filename);	 
							} else {
								echo $obj->gFormInsert('al_accomodations',$filename);	
							}
 						?>
                    <table boder="0" cellpadding="0" cellspacing="0" width="100%" class="generalinfo" style="margin-top:0px;">
                		...
                		<fieldset>
                    		...
                    		<input name="id" id="id" type="hidden" value="<?=$accid?>">
							<input name="add" id="add" type="button" size="" value="<?=($accid)?" save change ":" add "?>" onClick='<?=($accid)?"set_editData(\"al_accomodations\",$accid)":"set_insertData(\"al_accomodations\")"?>' > 
