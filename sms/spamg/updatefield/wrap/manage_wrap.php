...
$_COOKIE["topicdir"] = '<a href="javascript:;" onclick="gotoURL(\'../../index.php\')" class="top_menu_link">Preferences </a> > ' .
		...
		'Wrap > ';
$_COOKIE["topic"] = 'Wrap Infomation Manager';
...
$xml = "<command>".
 	"<table>db_trm</table>".
 	"<where name='trm_category_id' operator='='>5</where>".
	"<usejoin>no</usejoin>";
$sql = "select * from db_trm where trm_category_id='5' ";
$showInactive="";
$link="";
	...
	<tr>
    	<td class="content" width="100%">
					...
					<legend><b>Wrap Search</b></legend>
			           <table border="0" cellpadding="2" cellspacing="2">
                        <td height="30px">
     					...
     					&nbsp;&nbsp;
        			    <a href="javascript:;" onClick="gotoSearch('manage_wrap.php')" class="top_menu_link"><img src="../../../images/btn_search_bg.gif" alt="search" width="16" height="16" border="0"/></a>&nbsp; 
        			    <a href="javascript:;" onClick="gotoSearch('manage_wrap.php')" class="top_menu_link">Search</a> &nbsp;
        			    <a href="javascript:;" onClick="getReturnText('manage_wrap.php','page=1<?=(($_GET["show_inactive"])?"&show_inactive=true":"")?>','tableDisplay')" class="top_menu_link"><img src="../../../images/btn_show-all_bg.gif" alt="search" width="16" height="16" border="0"/></a>&nbsp; 
        			    <a href="javascript:;" onClick="getReturnText('manage_wrap.php','page=1<?=(($_GET["show_inactive"])?"&show_inactive=true":"")?>','tableDisplay')" class="top_menu_link">View All</a>
                        &nbsp;  &nbsp; <input id='show_inactive' type='checkbox' name='show_inactive' value='1' onClick="showInactive('manage_wrap.php')" <? echo ($_GET["show_inactive"])?"checked":""?> /> Show Inactive
                        </td></table>
     				</fieldset>
				</div> 
			...
			if(strlen($order)>0){
				...
				if($dir=='desc'){
					...
					for($i=0;$i<$column;$i++){
						if($arrFormType[$i]!="submit"&&$arrFormType[$i]!="button"&&$arrShowinList[$i]!="no"){
							$trmtype = ($arrFields[$i]=="".$element->activefield["name"])?"":" Wrap";
				...
				$chksql = "select * from db_trm where trm_category_id='5' ";
				...
				if($rs["rows"]>0){
					for($i=0;$i<$rs["rows"];$i++){
									...
									if($chkPageEdit){
										if($rs[$i]["$arrFields[$j]"]==1){
											$textout .=" \n<td style='padding-left:7px;padding-right:7px;' align='$align'>".
														"<a href=\"javascript:;\" onClick=\"javascript:setEnable('db_trm5',".$rs[$i]["trm_id"].",0);\" class=\"top_menu_link\">".
														"<img src='../../../images/active.png' border='0' title='active' /></a>".
														"</td>";
										}else{
											$textout .=" \n<td style='padding-left:7px;padding-right:7px;' align='$align'>".
														"<a href=\"javascript:;\" onClick=\"javascript:setEnable('db_trm5',".$rs[$i]["trm_id"].",1);\" class=\"top_menu_link\">".
						...
						if($chkPageEdit){
							$textout .= " \n".'<td><a href="javascript:;" onclick="editData(\''.$tbname.'5\','.$rs[$i]["trm_id"].')">update</a></td></tr>';					
						}else{
							$textout .="</tr>";
						}
			...
			<table cellspacing="0" border="0" cellpadding="0" id="infoheader" width="100%">
				<tr><td align="left"> <? if(!$rs1){echo ' 0';}echo " ".$rs1["rows"]." Record Total"; ?></td>
 				...
 				<td id="addinfo" align="right" width="50%">
 					<a href="javascript:;" onClick="getReturnText('add_wrap.php','page=1','tableDisplay');" class="top_menu_link">
                	Add New Wrap</a>&nbsp;&nbsp;<a href="javascript:;" onClick="getReturnText('add_wrap.php','page=1','tableDisplay');" class="top_menu_link"><img src="../../../images/addIcon.png" alt="Add" width="16" height="16" border="0"/></a>
                </td>
                <?}?>
                </tr>
 			</table>
