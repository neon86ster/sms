        	...
        	<div id="gform">
        		        <? 
							if($categoryid){
									...
									"<where name='gifttype_id' operator='='>$categoryid</where>" .
									"</command>";
								echo $obj->gFormEdit($xml,$filename);	 
							} else {
								echo $obj->gFormInsert('gl_gifttype',$filename);	
							}
 						?>
                    <table boder="0" cellpadding="0" cellspacing="0" width="100%" class="generalinfo" style="margin-top:0px;">
                		...
                		<fieldset>
							...
							<input name="add" id="add" type="button" size="" value="<?=($categoryid)?" save change ":" add "?>" onClick='<?=($categoryid)?"set_editData(\"gl_gifttype\",$categoryid)":"set_insertData(\"gl_gifttype\")"?>' > 
