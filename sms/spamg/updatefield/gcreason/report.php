...
$order=$obj->getParameter("order","gifttype_name");
...
if(!$showinactive){
$sql .= "where gifttype_active=1 " .
		"and (lower(gifttype_name) like '%$search%') ";
}else{
$sql .= "where lower(gifttype_name) like '%$search%' ";
}
...
for($i=0;$i<$rs["rows"];$i++){
	...
	for($j=0;$j<$column;$j++){
		if($arrShowinList[$j]!="no") {
				...
				if($arrFields[$j]=='gifttype_active') {
					if($chkPageEdit){
						if($rs[$i]["gifttype_active"]==1){
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('gl_gifttype',".$rs[$i]["gifttype_id"].",0);\"\>".
								 "<img src=\"/images/active.png\" border=\"0\" title=\"active\" /></a>";
						}else{
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('gl_gifttype',".$rs[$i]["gifttype_id"].",1);\">".
				...
				}else if($arrFields[$j]=='gifttype_name'){
...
<?		}
	...
	if($chkPageEdit){
		echo "<td class=\"report\"><a href=\"javascript:;\" onclick=\"editData('$tbname','".$rs[$i]["gifttype_id"]."')\">update</a></td>";
	}
	echo "</tr>";
}
if(!$rs["rows"]){}
