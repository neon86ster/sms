...
$order=$obj->getParameter("order","pay_name");
...
//query language
$sql = "select * from l_paytype ";
$search = strtolower($search);
// specific $search don't care about category select
if(!$showinactive){
$sql .= "where pay_active=1 " .
		"and (lower(pay_name) like '%$search%') ";
}else{
$sql .= "where lower(pay_name) like '%$search%' ";
}
...
for($i=0;$i<$rs["rows"];$i++){
	...
	for($j=0;$j<$column;$j++){
		if($arrShowinList[$j]!="no") {
				...
				if($arrFields[$j]=='pay_active') {
					if($chkPageEdit){
						if($rs[$i]["pay_active"]==1){
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('l_paytype',".$rs[$i]["pay_id"].",0);\"\>".
								 "<img src=\"/images/active.png\" border=\"0\" title=\"active\" /></a>";
						}else{
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('l_paytype',".$rs[$i]["pay_id"].",1);\">".
				...
				}else if($arrFields[$j]=='pay_category_id') {
					$data=$obj->getIdToText($rs[$i]['pay_category_id'],"l_paytype_category","pay_category_name","pay_category_id");
				}else if($arrFields[$j]=='pay_account_id') {
					$data=$obj->getIdToText($rs[$i]['pay_account_id'],"l_account","pay_account_name","pay_account_id");
				}
				else if($arrFields[$j]=='pay_template_id') {
					$data=$obj->getIdToText($rs[$i]['pay_template_id'],"l_template","pay_template_name","pay_template_id");
				}
...
<?		}
	...
	if($chkPageEdit){
		echo "<td class=\"report\"><a href=\"javascript:;\" onclick=\"editData('$tbname','".$rs[$i]["pay_id"]."')\">update</a></td>";
	}
	echo "</tr>";
}
if(!$rs["rows"]){}
