    	...
    	<td width="100%">
        	...
        	<div id="gform">
        		        <? 
							if($csivid){
									$xml = "<command>" .
									"<table>fl_csi_value</table>" .
									"<where name='csiv_id' operator='='>$csivid</where>" .
									"</command>";
								echo $obj->gFormEdit($xml,$filename);	 
							} else {
								echo $obj->gFormInsert('fl_csi_value',$filename);	
							}
 						?>
                    <table boder="0" cellpadding="0" cellspacing="0" width="100%" class="generalinfo" style="margin-top:0px;">
                		...
                		<fieldset>
                    		...
                    		<input name="id" id="id" type="hidden" value="<?=$csivid?>">
							<input name="add" id="add" type="submit" size="" value="<?=($csivid)?" save change ":" add "?>" onClick='<?=($csivid)?"set_editData(\"fl_csi_value\",$csivid)":"set_insertData(\"fl_csi_value\")"?>' > 
