        	...
        	<div>
        		        <? 
							if($empid){
									...
									"<where name='emp_id' operator='='>$empid</where>" .
									"</command>";
								echo $obj->gFormEdit($xml,$filename);	 
							} else {
								echo $obj->gFormInsert('l_employee',$filename);	
							}
 						?>
                    <table boder="0" cellpadding="0" cellspacing="0" width="100%" class="generalinfo" style="margin-top:0px;">
                		...
                		<fieldset>
                    		...
                    		<input name="id" id="id" type="hidden" value="<?=$empid?>">
							<input name="add" id="add" type="button" size="" value="<?=($empid)?" save change ":" add "?>" onClick='<?=($empid)?"set_editData(\"l_employee\",$empid)":"set_insertData(\"l_employee\")"?>' > 
