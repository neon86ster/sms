...
$order=$obj->getParameter("order","pcms_percent");
...
//query language
$sql = "select * from al_percent_cms ";
$search = strtolower($search);
$searchsql = $obj->convert_char($search);
// specific $search don't care about category select
$sql .= "where (lower(pcms_percent) like '%$searchsql%' " .
		"or lower(pcms_detail) like '%$searchsql%') ";
if(!$showinactive){$sql .= "and pcms_active=1 ";}
...
for($i=0;$i<$rs["rows"];$i++){
	...
	for($j=0;$j<$column;$j++){
		if($arrShowinList[$j]!="no") {
				...
				if($arrFields[$j]=='pcms_active') {
					if($chkPageEdit){
						if($rs[$i]["pcms_active"]==1){
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('al_percent_cms',".$rs[$i]["pcms_id"].",0);\"\>".
								 	"<img src=\"/images/active.png\" border=\"0\" title=\"active\" /></a>";
						}else{
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('al_percent_cms',".$rs[$i]["pcms_id"].",1);\">".
				...
				}else if($arrFields[$j]=='pcms_percent'||$arrFields[$j]=='pcms_detail'){
...
<?		}
	...
	if($chkPageEdit){
		echo "<td class=\"report\"><a href=\"javascript:;\" onclick=\"editData('$tbname','".$rs[$i]["pcms_id"]."')\">update</a></td>";
	}
	echo "</tr>";
}
if(!$rs["rows"]){}
