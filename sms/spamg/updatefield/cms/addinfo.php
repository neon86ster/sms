    	...
    	<td width="100%">
        	...
        	<div id="gform">
        		        <? 
							if($pcmsid){
									$xml = "<command>" .
									"<table>al_percent_cms</table>" .
									"<where name='pcms_id' operator='='>$pcmsid</where>" .
									"</command>";
								echo $obj->gFormEdit($xml,$filename);	 
							} else {
								echo $obj->gFormInsert('al_percent_cms',$filename);	
							}
 						?>
                    <table boder="0" cellpadding="0" cellspacing="0" width="100%" class="generalinfo" style="margin-top:0px;">
                		...
                		<fieldset>
                    		...
                    		<input name="id" id="id" type="hidden" value="<?=$pcmsid?>">
							<input name="add" id="add" type="button" size="" value="<?=($pcmsid)?" save change ":" add "?>" onClick='<?=($pcmsid)?"set_editData(\"al_percent_cms\",$pcmsid)":"set_insertData(\"al_percent_cms\")"?>' > 
