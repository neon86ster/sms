...
$filename = "../object.xml";
$errormsg ="";
$categoryid = $obj->getParameter("id");
    	...
    	<td width="100%">
        	...
        	<div>
        		        <? 
							if($categoryid){
									$xml = "<command>" .
									"<table>cl_product_category</table>" .
									"<where name='pd_category_id' operator='='>$categoryid</where>" .
									"</command>";
								echo $obj->gFormEdit($xml,$filename);	 
							} else {
								echo $obj->gFormInsert('cl_product_category',$filename);	
							}
 						?>
                    <table boder="0" cellpadding="0" cellspacing="0" width="100%" class="generalinfo" style="margin-top:0px;">
                		...
                		<fieldset>
                    		...
                    		<input name="id" id="id" type="hidden" value="<?=$categoryid?>">
							<input name="add" id="add" type="button" size="" value="<?=($categoryid)?" save change ":" add "?>" onClick='<?=($categoryid)?"set_editData(\"cl_product_category\",$categoryid)":"set_insertData(\"cl_product_category\")"?>' > 
