...
//query language
$sql = "select * from cl_product_category ";
$search = strtolower($search);
if(!$showinactive){
$sql .= "where pd_category_active=1 " .
		"and (lower(pd_category_name) like '%$search%' "  .
		"or lower(REPLACE(pd_category_detail,\"[br]\",\"\")) like '%$search%') ";
}else{
$sql .= "where lower(pd_category_name) like '%$search%' " .
		"or lower(REPLACE(pd_category_detail,\"[br]\",\"\")) like '%$search%' ";
}
...
for($i=0;$i<$rs["rows"];$i++){
	...
	for($j=0;$j<$column;$j++){
		if($arrShowinList[$j]!="no") {
				...
				if($arrFields[$j]=='pd_category_active') {
					if($chkPageEdit){
						if($rs[$i]["pd_category_active"]==1){
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('cl_product_category',".$rs[$i]["pd_category_id"].",0);\"\>".
								 "<img src=\"/images/active.png\" border=\"0\" title=\"active\" /></a>";
						}else{
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('cl_product_category',".$rs[$i]["pd_category_id"].",1);\">".
				...
				}else if($arrFields[$j]=='set_commission') {	// can set this value when create product category only
						if($rs[$i]["set_commission"]==1){
							$data = "<img src=\"/images/active.png\" border=\"0\" title=\"active\" />";
						}else{
							$data = "<img src=\"/images/inactive.png\" border=\"0\" title=\"active\" />";
						}
				}else if($arrFields[$j]=='set_payment') {		// can set this value when create product category only
					if($rs[$i]["set_payment"]==1){
							$data = "<img src=\"/images/active.png\" border=\"0\" title=\"active\" />";
						}else{
							$data = "<img src=\"/images/inactive.png\" border=\"0\" title=\"active\" />";
						}
				}else if($arrFields[$j]=='set_gift') {		// can set this value when create product category only
					if($rs[$i]["set_gift"]==1){
							$data = "<img src=\"/images/active.png\" border=\"0\" title=\"active\" />";
						}else{
							$data = "<img src=\"/images/inactive.png\" border=\"0\" title=\"active\" />";
						}
				}else if($arrFields[$j]=='pos_neg_value') {		// can set this value when create product category only
					if($rs[$i]["pos_neg_value"]==1){
							$data = "<img src=\"/images/active.png\" border=\"0\" title=\"active\" />";
						}else{
							$data = "<img src=\"/images/inactive.png\" border=\"0\" title=\"active\" />";
						}
				}else if($arrFields[$j]=='plus_minus_value') {		// can set this value when create product category only
					if($rs[$i]["plus_minus_value"]==1){
							$data = "<img src=\"/images/active.png\" border=\"0\" title=\"active\" />";
						}else{
							$data = "<img src=\"/images/inactive.png\" border=\"0\" title=\"active\" />";
						}
				}else if($arrFields[$j]=='pd_category_detail'){
						$data = str_replace("[br]","<br>",$rs[$i]["$arrFields[$j]"]);
						$data = $obj->hightLightChar($search,$data);
				}else if($arrFields[$j]=='pd_category_name'){
...
<?		}
	...
	if($chkPageEdit){
		echo "<td class=\"report\"><a href=\"javascript:;\" onclick=\"editData('$tbname','".$rs[$i]["pd_category_id"]."')\">update</a></td>";
	}
	echo "</tr>";
}
if(!$rs["rows"]){}
