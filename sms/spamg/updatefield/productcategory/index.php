...
$order=$obj->getParameter("order","pd_category_name");
    	...
    	<td align="right" height="30px" class="rheader">
    		<table border="0" cellspacing="0" cellpadding="0">
			      <tr>
			        ...
			        <?	
			        	$chksql = "select pd_category_id from cl_product_category ";
			        	$search = strtolower($search);
			        	if(!$showinactive){
			        		$chksql .= "where pd_category_active=1 " .
			        				"and (lower(pd_category_name) like '%$search%' " .
			        				"or lower(REPLACE(pd_category_detail,\"[br]\",\"\")) like '%$search%') ";
			        	}else{
			        		$chksql .= "where lower(pd_category_name) like '%$search%' " .
			        				"or lower(REPLACE(pd_category_detail,\"[br]\",\"\")) like '%$search%' ";
			        	}
