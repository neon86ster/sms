...
$order=$obj->getParameter("order","category_name");
...
//query language
$sql = "select * from l_mkcode_category ";
$search = strtolower($search);
// specific $search don't care about category select
if(!$showinactive){
$sql .= "where category_active=1 " .
		"and (lower(category_name) like '%$search%') ";
}else{
$sql .= "where lower(category_name) like '%$search%' ";
}
...
for($i=0;$i<$rs["rows"];$i++){
	...
	for($j=0;$j<$column;$j++){
		if($arrShowinList[$j]!="no") {
				...
				if($arrFields[$j]=='category_active') {
					if($chkPageEdit){
						if($rs[$i]["category_active"]==1){
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('l_mkcode_category',".$rs[$i]["category_id"].",0);\"\>".
								 "<img src=\"/images/active.png\" border=\"0\" title=\"active\" /></a>";
						}else{
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('l_mkcode_category',".$rs[$i]["category_id"].",1);\">".
				...
				}else if($arrFields[$j]=='category_name'){
...
<?		}
	...
	if($chkPageEdit){
		echo "<td class=\"report\"><a href=\"javascript:;\" onclick=\"editData('$tbname','".$rs[$i]["category_id"]."')\">update</a></td>";
	}
	echo "</tr>";
}
if(!$rs["rows"]){}
