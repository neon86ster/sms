    	...
    	<td width="100%">
        	...
        	<div>
        		        <? 
							if($categoryid){
									$xml = "<command>" .
									"<table>l_mkcode_category</table>" .
									"<where name='category_id' operator='='>$categoryid</where>" .
									"</command>";
								echo $obj->gFormEdit($xml,$filename);	 
							} else {
								echo $obj->gFormInsert('l_mkcode_category',$filename);	
							}
 						?>
                    <table boder="0" cellpadding="0" cellspacing="0" width="100%" class="generalinfo" style="margin-top:0px;">
                		...
                		<fieldset>
							...
							<input name="add" id="add" type="button" size="" value="<?=($categoryid)?" save change ":" add "?>" onClick='<?=($categoryid)?"set_editData(\"l_mkcode_category\",$categoryid)":"set_insertData(\"l_mkcode_category\")"?>' > 
