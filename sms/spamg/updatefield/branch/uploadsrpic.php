...
if(isset( $_REQUEST["Submit"] )) { 	
	...
	if($rs["rows"]==1){
		$ints = "sr_".$rs[0]["branch_name"].$rs[0]["branch_id"];
	}else{
		$ints = "sr_tmp";
	}
	//print_r($_FILES);
		...
		if (!empty($imgsn))
		{
			$_SESSION["image_sr"]=$imgsn;
			$target_path=$customize_img ."/images/branch/";
 			move_uploaded_file ($_FILES['uploadedfile']['tmp_name'],$target_path.$imgsn) ; 
			
			//exit();
			resizeImageE($target_path.$imgsn,$target_path,"100","91");
			if($bid){$id = $obj->setResult("update bl_branchinfo set sr_logo='$imgsn' where branch_id=$bid");}
			
			$uploadsuccess = true;
?>
<script type="text/javascript">
	function setVisibleC() {
		window.opener.document.getElementById('images_sr').src="<?=$customize_part?>/images/branch/<?=empty($_SESSION["image_sr"]) ? "sr_tmp.png" : $_SESSION["image_sr"] ?>";
	}
</script>
<img src="<?=$customize_part?>/images/branch/<?=empty($_SESSION["image_sr"]) ? "sr_tmp.png" : $_SESSION["image_sr"] ?>">
<?php
		}
		else
...
<input type="submit" name="Submit" value="Upload File" /> <br />(Sale Receipt logo should be in (PNG, GIF, JPG) format, 100 × 91 Pixel)
</form>
<?if($uploadsuccess){?>
<script type="text/javascript">
	setVisibleC();
