...
$order=$obj->getParameter("order","package_name");
	...
	for($j=0;$j<$column;$j++){
		if($arrShowinList[$j]!="no") {
				...
				if($arrFields[$j]=='package_active') {
					if($chkPageEdit){
						if($rs[$i]["package_active"]==1){
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('db_package',".$rs[$i]["package_id"].",0);\"\>".
								 "<img src=\"/images/active.png\" border=\"0\" title=\"active\" /></a>";
						}else{
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('db_package',".$rs[$i]["package_id"].",1);\">".
						...
						/*$chksql = "select trm_category_id,trm_name from db_trm_package " .
									"where package_id=".$rs[$i]["package_id"]." " .
									"order by db_trm.trm_category_id asc ";
						$chkrs = $obj->getResult($chksql);
						$trm = '';
						if($chkrs["rows"]>0){
								for($k=0;$k<$chkrs["rows"];$k++){
									$trmcname = $obj->getIdToText($chkrs[$k]["trm_category_id"],"db_trm_category","trm_category_name","trm_category_id");
									$trm .= " ".$trmcname." - ".$chkrs[$k]["trm_name"]."<br/>";
								}
						} else {$trm .= "No treatment in this package!!";}
						$data = $obj->hightLightChar($search,$rs[$i]["$arrFields[$j]"]);
						$data = "<table border=\"0\" width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" class=\"generalinfo\" style=\"padding: 0 0 0 0;margin-left:0px;\">\n" .
									"<tr>\n<td height=\"18\">$data</td>\n" .
									"<td align=\"right\">\n" .
									"&nbsp;&nbsp;" .
									"<span title=\" header=[Massage Detail] body=['".$obj->encodeText($trm)."']\" " .
...
<?		}
	...
	if($chkPageEdit){
		echo "<td class=\"report\"><a href=\"javascript:;\" onclick=\"editData('$tbname','".$rs[$i]["package_id"]."')\">update</a></td>";
	}
	echo "</tr>";
}
if(!$rs["rows"]){}
