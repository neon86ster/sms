...
$filename = "../object.xml";
$errormsg ="";
$departmentid = $obj->getParameter("id");
...
//set avaliable emp_department number
    	...
    	<td width="100%">
        	...
        	<div>
        		        <? 
							if($departmentid){
									$xml = "<command>" .
									"<table>l_employee_department</table>" .
									"<where name='emp_department_id' operator='='>$departmentid</where>" .
									"</command>";
								echo $obj->gFormEdit($xml,$filename);	 
							} else {
								echo $obj->gFormInsert('l_employee_department',$filename);	
							}
 						?>
                    <table boder="0" cellpadding="0" cellspacing="0" width="100%" class="generalinfo" style="margin-top:0px;">
                		...
                		<fieldset>
                    		...
                    		<input name="id" id="id" type="hidden" value="<?=$departmentid?>">
							<input name="add" id="add" type="button" size="" value="<?=($departmentid)?" save change ":" add "?>" onClick='<?=($departmentid)?"set_editData(\"l_employee_department\",$departmentid)":"set_insertData(\"l_employee_department\")"?>' > 
