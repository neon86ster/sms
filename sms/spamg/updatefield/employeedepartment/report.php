...
$order=$obj->getParameter("order","emp_department_name");
...
//query language
$sql = "select * from l_employee_department ";
$search = strtolower($search);
// specific $search don't care about category select
$sql .= "where lower(emp_department_name) like '%$search%' ";
if(!$showinactive){$sql .= "and emp_department_active=1 ";}
...
for($i=0;$i<$rs["rows"];$i++){
	...
	for($j=0;$j<$column;$j++){
		if($arrShowinList[$j]!="no") {
				...
				if($arrFields[$j]=='emp_department_active') {
					if($chkPageEdit&&$rs[$i]["emp_department_id"]>4){
						if($rs[$i]["emp_department_active"]==1){
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('l_employee_department',".$rs[$i]["emp_department_id"].",0);\"\>".
								 	"<img src=\"/images/active.png\" border=\"0\" title=\"active\" /></a>";
						}else{
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('l_employee_department',".$rs[$i]["emp_department_id"].",1);\">".
				...
				}else if($arrFields[$j]=='emp_department_name'){
...
<?		}
	...
	if($chkPageEdit){
		if($rs[$i]["emp_department_id"]>4){
			echo "<td class=\"report\"><a href=\"javascript:;\" onclick=\"editData('$tbname','".$rs[$i]["emp_department_id"]."')\">update</a></td>";
		}else{
