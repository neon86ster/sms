        	...
        	<div>
        		        <? 
							if($cityid){
									$xml = "<command>" .
									"<table>al_city</table>" .
									"<where name='city_id' operator='='>$cityid</where>" .
									"</command>";
								echo $obj->gFormEdit($xml,$filename);	 
							} else {
								echo $obj->gFormInsert('al_city',$filename);	
							}
 						?>
                    <table boder="0" cellpadding="0" cellspacing="0" width="100%" class="generalinfo" style="margin-top:0px;">
                		...
                		<fieldset>
                    		...
                    		<input name="id" id="id" type="hidden" value="<?=$cityid?>">
							<input name="add" id="add" type="button" size="" value="<?=($cityid)?" save change ":" add "?>" onClick='<?=($cityid)?"set_editData(\"al_city\",$cityid)":"set_insertData(\"al_city\")"?>' > 
