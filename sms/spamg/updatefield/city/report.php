...
$order=$obj->getParameter("order","city_name");
...
$sql .= "where (lower(city_name) like '%$search%' " .
		"or lower(city_code) like '%$search%') ";
if(!$showinactive){$sql .= "and city_active=1 ";}
...
for($i=0;$i<$rs["rows"];$i++){
	...
	for($j=0;$j<$column;$j++){
		if($arrShowinList[$j]!="no") {
				...
				if($arrFields[$j]=='city_active') {
					if($chkPageEdit){
						if($rs[$i]["city_active"]==1){
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('al_city',".$rs[$i]["city_id"].",0);\"\>".
								 	"<img src=\"/images/active.png\" border=\"0\" title=\"active\" /></a>";
						}else{
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('al_city',".$rs[$i]["city_id"].",1);\">".
				...
				}else if($arrFields[$j]=='city_name'||$arrFields[$j]=='city_code'){
...
<?		}
	...
	if($chkPageEdit){
		echo "<td class=\"report\"><a href=\"javascript:;\" onclick=\"editData('$tbname','".$rs[$i]["city_id"]."')\">update</a></td>";
	}
	echo "</tr>";
}
if(!$rs["rows"]){}
