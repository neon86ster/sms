...
$_COOKIE["topicdir"] = '<a href="javascript:;" onclick="gotoURL(\'../../index.php\')" class="top_menu_link">Preferences </a> > ' .
		...
		'<a href="javascript:;" onclick="gotoURL(\'index.php\')" class="top_menu_link"> Massage </a> > ';
if(isset($_POST["id"])){
	$_COOKIE["topic"] = 'Edit Massage Infomation';
} else if(!isset($_POST["id"])) {
	$_COOKIE["topic"] = 'Add Massage Infomation';
}
...
if($_POST["method"]=="setactive" && $hasSession){
	...
	if($name!=false){
		...
		header("Location: manage_massage.php?msg=$successmsg");
	} else {
...
if($_POST["add"] == " save change " && $hasSession){
	...
	if($id){
		$obj->setResult("update db_trm set trm_category_id=3 where trm_id=".$id);
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
    	<td class="content" width="100%">
        	...
        	<div>
    			<fieldset>
					<legend><b>Massage Information</b></legend>
                ...
                <fieldset>
					...
					<input name="trmCategoryId" id="trmCategoryId" type="hidden" value="3">
					<input name="id" id="id" type="hidden" value="<?=$_POST["id"]?>">
					<input type="hidden" name="phpSql" id="phpSql" value="<?=$_POST["phpSql"]?>"/>
					<input name="add" id="add" type="submit" size="" value="<?=(isset($_POST["id"]))?" save change ":" add "?>" onClick='<?=(isset($_POST["id"]))?"set_editData(\"db_trm3\",".$_POST["id"].")":"set_insertData(\"db_trm3\")"?>' >&nbsp; 
					<input name="cancel" id="cancel" type="submit" size="" value=" cancel " onClick="getReturnText('manage_massage.php','<?=$_POST["phpSql"]?>','tableDisplay');" >  
                </fieldset>
			</div>
		</td>
    </tr>
