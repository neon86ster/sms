...
if($add){
		...
		$chkid = $obj->getIdToText($_REQUEST["trm_name"],"db_trm","trm_id","trm_name","trm_category_id=3 $condition");
		if($chkid>0){
		 	$obj->setErrorMsg("This Massage Type is already have in the system. please insert new name!!'");
		 	$errormsg = $obj->getErrorMsg();
	 		$add = false;
		 }
}
...
if($add == " save change " && $chkPageEdit){
	...
	if($id){
		$chksql = "update db_trm set trm_category_id=3 where trm_id=$id";
