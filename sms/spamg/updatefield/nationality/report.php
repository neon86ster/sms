...
$order=$obj->getParameter("order","nationality_name");
...
//query language
$sql = "select * from dl_nationality ";
$search = strtolower($search);
// specific $search don't care about category select
if(!$showinactive){
$sql .= "where nationality_active=1 " .
		"and (lower(nationality_name) like '%$search%') ";
}else{
$sql .= "where lower(nationality_name) like '%$search%' ";
}
...
for($i=0;$i<$rs["rows"];$i++){
	...
	for($j=0;$j<$column;$j++){
		if($arrShowinList[$j]!="no") {
				...
				if($arrFields[$j]=='nationality_active') {
					if($chkPageEdit){
						if($rs[$i]["nationality_active"]==1){
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('dl_nationality',".$rs[$i]["nationality_id"].",0);\"\>".
								 "<img src=\"/images/active.png\" border=\"0\" title=\"active\" /></a>";
						}else{
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('dl_nationality',".$rs[$i]["nationality_id"].",1);\">".
				...
				}else if($arrFields[$j]=='continent_id') {
					$data=$obj->getIdToText($rs[$i]['continent_id'],"dl_continent","continent_name","continent_id");
				}else if($arrFields[$j]=='nationality_name'){
...
<?		}
	...
	if($chkPageEdit){
		echo "<td class=\"report\"><a href=\"javascript:;\" onclick=\"editData('$tbname','".$rs[$i]["nationality_id"]."')\">update</a></td>";
	}
	echo "</tr>";
}
if(!$rs["rows"]){}
