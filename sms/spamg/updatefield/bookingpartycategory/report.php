...
$order=$obj->getParameter("order","bp_category_name");
...
//query language
$sql = "select * from al_bookparty_category ";
$search = strtolower($search);
// specific $search don't care about category select
if(!$showinactive){
$sql .= "where bp_category_active=1 " .
		"and (lower(bp_category_name) like '%$search%' "  .
		"or lower(REPLACE(bp_category_detail,\"[br]\",\"\")) like '%$search%') ";
}else{
$sql .= "where lower(bp_category_name) like '%$search%' " .
		"or lower(REPLACE(bp_category_detail,\"[br]\",\"\")) like '%$search%' ";
}
...
for($i=0;$i<$rs["rows"];$i++){
	...
	for($j=0;$j<$column;$j++){
		if($arrShowinList[$j]!="no") {
				...
				if($arrFields[$j]=='bp_category_active') {
					if($chkPageEdit){
						if($rs[$i]["bp_category_active"]==1){
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('al_bookparty_category',".$rs[$i]["bp_category_id"].",0);\"\>".
								 "<img src=\"/images/active.png\" border=\"0\" title=\"active\" /></a>";
						}else{
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('al_bookparty_category',".$rs[$i]["bp_category_id"].",1);\">".
				...
				}else if($arrFields[$j]=='bp_category_detail'){
						$data = str_replace("[br]","<br>",$rs[$i]["$arrFields[$j]"]);
						$data = $obj->hightLightChar($search,$data);
				}else if($arrFields[$j]=='bp_category_name'){
...
<?		}
	...
	if($chkPageEdit){
		echo "<td class=\"report\"><a href=\"javascript:;\" onclick=\"editData('$tbname','".$rs[$i]["bp_category_id"]."')\">update</a></td>";
	}
	echo "</tr>";
}
if(!$rs["rows"]){}
