        	...
        	<div>
        		        <? 
							if($categoryid){
									...
									"<where name='bp_category_id' operator='='>$categoryid</where>" .
									"</command>";
								echo $obj->gFormEdit($xml,$filename);	 
							} else {
								echo $obj->gFormInsert('al_bookparty_category',$filename);	
							}
 						?>
                    <table boder="0" cellpadding="0" cellspacing="0" width="100%" class="generalinfo" style="margin-top:0px;">
                		...
                		<fieldset>
							...
							<input name="add" id="add" type="button" size="" value="<?=($categoryid)?" save change ":" add "?>" onClick='<?=($categoryid)?"set_editData(\"al_bookparty_category\",$categoryid)":"set_insertData(\"al_bookparty_category\")"?>' > 
