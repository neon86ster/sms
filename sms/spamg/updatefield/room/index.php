...
<form name="rooms" id="rooms" action="" method="post" style="padding:0;margin:0">
    	...
    	<td valign="top" height="30px" colspan="2" class="rheader">
    		<table border="0" cellspacing="0" cellpadding="0">
			      <tr>
			        ...
			        Branch: <?=$obj->makeListbox("categoryid","all_bl_branchinfo","branch_name","branch_id",$categoryid,0,"branch_name","branch_active","1","branch_name != \"All\"",0,0,"sortInfo('','1');")?>
    	...
    	<td align="right" height="30px" class="rheader">
    		<table border="0" cellspacing="0" cellpadding="0">
			      <tr>
			        ...
			        <?	
			        	$sql = "select room_id from bl_room ";
			        	$search = strtolower($search);
						// specific $search don't care about category select
			        	if(!$showinactive){
							$sql .= "where room_active=1 ";
						if($categoryid>1){$sql .= "and branch_id=$categoryid ";}
							$sql .= "and (lower(room_name) like '%$search%' "  .
								"or lower(REPLACE(room_detail,\"[br]\",\"\")) like '%$search%') ";
						}else{
							if($categoryid>1){
								$sql .= "where branch_id=$categoryid " .
									"and (lower(room_name) like '%$search%' "  .
									"or lower(REPLACE(room_detail,\"[br]\",\"\")) like '%$search%') ";
							}else{
								$sql .= "where lower(room_name) like '%$search%' "  .
								"or lower(REPLACE(room_detail,\"[br]\",\"\")) like '%$search%' ";
							}
						}
