...
for($i=0;$i<$rs["rows"];$i++){
	...
	for($j=0;$j<$column;$j++){
		if($arrShowinList[$j]!="no") {
				...
				}else if($arrFields[$j]=='room_active') {
					if($chkPageEdit){
						if($rs[$i]["room_active"]==1){
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('bl_room',".$rs[$i]["room_id"].",0);\"\>".
								 "<img src=\"/images/active.png\" border=\"0\" title=\"active\" /></a>";
						}else{
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('bl_room',".$rs[$i]["room_id"].",1);\">".
...
<?		}
	...
	if($chkPageEdit){
		echo "<td class=\"report\"><a href=\"javascript:;\" onclick=\"editData('$tbname','".$rs[$i]["room_id"]."')\">update</a></td>";
	}
	echo "</tr>";
}
if(!$rs["rows"]){}
