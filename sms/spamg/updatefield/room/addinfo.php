    			...
    			<table style="border: solid 3px #ff0000;" width="100%" cellspacing="0" cellpadding="10">
    				<tr>
        		        ...
        		        <? 
							if($roomid){
									$xml = "<command>" .
									"<table>bl_room</table>" .
									"<where name='room_id' operator='='>$roomid</where>" .
									"</command>";
								echo $obj->gFormEdit($xml,$filename);	 
							} else {
								echo $obj->gFormInsert('bl_room',$filename);	
							}
 						?>
                    <table boder="0" cellpadding="0" cellspacing="0" width="100%" class="generalinfo" style="margin-top:0px;">
                		...
                		<fieldset>
							...
							<input name="add" id="add" type="button" size="" value="<?=($roomid)?" save change ":" add "?>" onClick='<?=($roomid)?"set_editData(\"bl_room\",$roomid)":"set_insertData(\"bl_room\")"?>' > 
