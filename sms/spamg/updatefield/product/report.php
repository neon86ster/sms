...
$order=$obj->getParameter("order","pd_name");
...
if(!$showinactive){
$sql .= "where pd_active=1 ";
if($categoryid){$sql .= "and pd_category_id=$categoryid ";}
$sql .= "and (lower(pd_name) like '%$search%' "  .
		"or lower(standard_price) like '%$search%') ";
}else{
	if($categoryid){
		$sql .= "where pd_category_id=$categoryid " .
			"and (lower(pd_name) like '%$search%' "  .
			"or lower(standard_price) like '%$search%') ";
	}else{
		$sql .= "where lower(pd_name) like '%$search%' "  .
		"or lower(standard_price) like '%$search%' ";
	}
}
	...
	for($j=0;$j<$column;$j++){
		if($arrShowinList[$j]!="no") {
				...
				if($arrFields[$j]=='pd_category_id') {
					...
					if($chkPageEdit){
						if($rs[$i]["pd_active"]==1){
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('cl_product',".$rs[$i]["pd_id"].",0);\"\>".
								 "<img src=\"/images/active.png\" border=\"0\" title=\"active\" /></a>";
						}else{
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('cl_product',".$rs[$i]["pd_id"].",1);\">".
					...
					if($chkPageEdit){
						if($rs[$i]["set_tax"]==1){
							$data = "<a href=\"javascript:;\" onClick=\"setTax('cl_product',".$rs[$i]["pd_id"].",0);\"\>".
								 "<img src=\"/images/active.png\" border=\"0\" title=\"active\" /></a>";
						}else{
							$data = "<a href=\"javascript:;\" onClick=\"setTax('cl_product',".$rs[$i]["pd_id"].",1);\">".
									"<img src=\"/images/inactive.png\" border=\"0\" title=\"expired\" /></a>";
						}
					}else{
						if($rs[$i]["set_tax"]==1){
					...
					if($chkPageEdit){
						if($rs[$i]["set_sc"]==1){
							$data = "<a href=\"javascript:;\" onClick=\"setSc('cl_product',".$rs[$i]["pd_id"].",0);\"\>".
								 "<img src=\"/images/active.png\" border=\"0\" title=\"active\" /></a>";
						}else{
							$data = "<a href=\"javascript:;\" onClick=\"setSc('cl_product',".$rs[$i]["pd_id"].",1);\">".
									"<img src=\"/images/inactive.png\" border=\"0\" title=\"expired\" /></a>";
						}
					}else{
						if($rs[$i]["set_sc"]==1){
				...
				}else if($arrFields[$j]=='member_takeout'){
					if($chkPageEdit){
						if($rs[$i]["member_takeout"]==1){
							$data = "<a href=\"javascript:;\" onClick=\"setmember_takeout('cl_product',".$rs[$i]["pd_id"].",0);\"\>".
								 "<img src=\"/images/active.png\" border=\"0\" title=\"active\" /></a>";
						}else{
							$data = "<a href=\"javascript:;\" onClick=\"setmember_takeout('cl_product',".$rs[$i]["pd_id"].",1);\">".
									"<img src=\"/images/inactive.png\" border=\"0\" title=\"expired\" /></a>";
						}
					}else{
						if($rs[$i]["member_takeout"]==1){
...
<?		}
	...
	if($chkPageEdit){
		echo "<td class=\"report\"><a href=\"javascript:;\" onclick=\"editData('$tbname','".$rs[$i]["pd_id"]."')\">update</a></td>";
	}
	echo "</tr>";
}
if(!$rs["rows"]){}
