...
$querystr = "&pageid=$pageid&search=$searchstr&order=$order&page=$page&sort=$sort&showinactive=$showinactive&categoryid=".$categoryid;
//set product active
...
//set product tax
if($method=="settax" && $chkPageEdit){
	$sql = "";
	$active = $obj->getParameter("active");
	$name = $obj->setTax($_REQUEST,$filename);
	if($name!=false){
		if($active==1){
...
//set product servicecharge
if($method=="setsc" && $chkPageEdit){
	$sql = "";
	$active = $obj->getParameter("active");
	$name = $obj->setSc($_REQUEST,$filename);
	if($name!=false){
		if($active==1){
...
if($method=="setmember_takeout" && $chkPageEdit){
	$sql = "";
	$active = $obj->getParameter("active");
	$name = $obj->setmember_takeout($_REQUEST,$filename);
	if($name!=false){
		if($active==1){
			$successmsg="$name Member Take Out is set!!";
		}else{
			$successmsg="$name Member Take Out is unset!!";
		}
		$successmsg=$successmsg.$querystr;
		header("Location: index.php?msg=$successmsg");
	} else {
        	...
        	<div>
        		        <? 
							if($roomid){
									...
									"<where name='pd_id' operator='='>$roomid</where>" .
									"</command>";
                    ...
                    <table boder="0" cellpadding="0" cellspacing="0" width="100%" class="generalinfo" style="margin-top:0px;">
                		...
                		<fieldset>
                    		...
                    		<input name="id" id="id" type="hidden" value="<?=$roomid?>">
							<input name="add" id="add" type="button" size="" value="<?=($roomid)?" save change ":" add "?>" onClick='<?=($roomid)?"set_editData(\"cl_product\",$roomid)":"set_insertData(\"cl_product\")"?>' > 
