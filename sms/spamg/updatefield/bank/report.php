...
$order=$obj->getParameter("order","bank_Ename");
...
//query language
$sql = "select * from l_bankname ";
$search = strtolower($search);
// specific $search don't care about category select
$sql .= "where (lower(bank_Ename) like '%$search%' " .
		"or lower(bank_Tname) like '%$search%') ";
if(!$showinactive){$sql .= "and bank_active=1 ";}
...
for($i=0;$i<$rs["rows"];$i++){
	...
	for($j=0;$j<$column;$j++){
		if($arrShowinList[$j]!="no") {
				...
				if($arrFields[$j]=='bank_active') {
					if($chkPageEdit){
						if($rs[$i]["bank_active"]==1){
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('l_bankname',".$rs[$i]["bank_id"].",0);\"\>".
								 	"<img src=\"/images/active.png\" border=\"0\" title=\"active\" /></a>";
						}else{
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('l_bankname',".$rs[$i]["bank_id"].",1);\">".
				...
				}else if($arrFields[$j]=='bank_Ename'||$arrFields[$j]=='bank_Tname'){
...
<?		}
	...
	if($chkPageEdit){
		echo "<td class=\"report\"><a href=\"javascript:;\" onclick=\"editData('$tbname','".$rs[$i]["bank_id"]."')\">update</a></td>";
	}
	echo "</tr>";
}
if(!$rs["rows"]){}
