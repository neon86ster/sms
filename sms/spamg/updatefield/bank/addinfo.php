    	...
    	<td width="100%">
        	...
        	<div>
        		        <? 
							if($bankid){
									$xml = "<command>" .
									"<table>l_bankname</table>" .
									"<where name='bank_id' operator='='>$bankid</where>" .
									"</command>";
								echo $obj->gFormEdit($xml,$filename);	 
							} else {
								echo $obj->gFormInsert('l_bankname',$filename);	
							}
 						?>
                    <table boder="0" cellpadding="0" cellspacing="0" width="100%" class="generalinfo" style="margin-top:0px;">
                		...
                		<fieldset>
                    		...
                    		<input name="id" id="id" type="hidden" value="<?=$bankid?>">
							<input name="add" id="add" type="button" size="" value="<?=($bankid)?" save change ":" add "?>" onClick='<?=($bankid)?"set_editData(\"l_bankname\",$bankid)":"set_insertData(\"l_bankname\")"?>' > 
