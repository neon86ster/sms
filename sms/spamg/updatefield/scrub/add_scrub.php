...
$_COOKIE["topicdir"] = '<a href="javascript:;" onclick="gotoURL(\'../../index.php\')" class="top_menu_link">Preferences </a> > ' .
		...
		'<a href="javascript:;" onclick="gotoURL(\'index.php\')" class="top_menu_link">Scrub </a> > ';
if(isset($_POST["id"])){
	$_COOKIE["topic"] = 'Edit Scrub Infomation';
} else if(!isset($_POST["id"])) {
	$_COOKIE["topic"] = 'Add Scrub Infomation';
}
...
if($_POST["method"]=="setactive" && $hasSession){
	...
	if($name!=false){
		...
		header("Location: manage_scrub.php?msg=$successmsg");
	} else {
...
if($_POST["add"] == " save change " && $hasSession){
	...
	if($id){
		$obj->setResult("update db_trm set trm_category_id=4 where trm_id=".$id);
    	...
    	<td class="content" width="100%">
        	...
        	<div>
                ...
                <fieldset>
					...
					<input name="trmCategoryId" id="trmCategoryId" type="hidden" value="4">
					<input name="id" id="id" type="hidden" value="<?=$_POST["id"]?>">
					<input type="hidden" name="phpSql" id="phpSql" value="<?=$_POST["phpSql"]?>"/>
					<input name="add" id="add" type="submit" size="" value="<?=(isset($_POST["id"]))?" save change ":" add "?>" onClick='<?=(isset($_POST["id"]))?"set_editData(\"db_trm4\",".$_POST["id"].")":"set_insertData(\"db_trm4\")"?>' >&nbsp; 
					<input name="cancel" id="cancel" type="submit" size="" value=" cancel " onClick="getReturnText('manage_scrub.php','<?=$_POST["phpSql"]?>','tableDisplay');" >  
                </fieldset>
			</div>
		</td>
    </tr>
