...
$order=$obj->getParameter("order","pay_account_name");
...
//query language
$sql = "select * from l_account ";
$search = strtolower($search);
// specific $search don't care about category select
if(!$showinactive){
$sql .= "where account_active=1 " .
		"and (lower(pay_account_name) like '%$search%') ";
}else{
$sql .= "where lower(pay_account_name) like '%$search%' ";
}
...
for($i=0;$i<$rs["rows"];$i++){
	...
	for($j=0;$j<$column;$j++){
		if($arrShowinList[$j]!="no") {
				...
				if($arrFields[$j]=='account_active') {
					if($chkPageEdit){
						if($rs[$i]["account_active"]==1){
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('l_account',".$rs[$i]["pay_account_id"].",0);\"\>".
								 "<img src=\"/images/active.png\" border=\"0\" title=\"active\" /></a>";
						}else{
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('l_account',".$rs[$i]["pay_account_id"].",1);\">".
				...
				}else if($arrFields[$j]=='pay_account_name'){
						$data = $obj->hightLightChar($search,$rs[$i]["$arrFields[$j]"]);
...
<?		}
	...
	if($chkPageEdit){
		echo "<td class=\"report\"><a href=\"javascript:;\" onclick=\"editData('$tbname','".$rs[$i]["pay_account_id"]."')\">update</a></td>";
	}
	echo "</tr>";
}
if(!$rs["rows"]){}
