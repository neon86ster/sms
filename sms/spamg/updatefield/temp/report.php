...
$order=$obj->getParameter("order","pay_template_name");
...
//query language
$sql = "select * from l_template ";
$search = strtolower($search);
// specific $search don't care about category select
if(!$showinactive){
$sql .= "where template_active=1 " .
		"and (lower(pay_template_name) like '%$search%') ";
}else{
$sql .= "where lower(pay_template_name) like '%$search%' ";
}
...
for($i=0;$i<$rs["rows"];$i++){
	...
	for($j=0;$j<$column;$j++){
		if($arrShowinList[$j]!="no") {
				...
				if($arrFields[$j]=='template_active') {
					if($chkPageEdit){
						if($rs[$i]["template_active"]==1){
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('l_template',".$rs[$i]["pay_template_id"].",0);\"\>".
								 "<img src=\"/images/active.png\" border=\"0\" title=\"active\" /></a>";
						}else{
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('l_template',".$rs[$i]["pay_template_id"].",1);\">".
				...
				}else if($arrFields[$j]=='pay_template_name'){
						$data = $obj->hightLightChar($search,$rs[$i]["$arrFields[$j]"]);
...
<?		}
	...
	if($chkPageEdit){
		echo "<td class=\"report\"><a href=\"javascript:;\" onclick=\"editData('$tbname','".$rs[$i]["pay_template_id"]."')\">update</a></td>";
	}
	echo "</tr>";
}
if(!$rs["rows"]){}
