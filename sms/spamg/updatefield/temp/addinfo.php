        	...
        	<div>
        		        <? 
							if($nationid){
									...
									$xml = "<command>" .
									"<table>l_template</table>" .
									"<where name='pay_template_id' operator='='>$nationid</where>" .
									"</command>";
								echo $obj->gFormEdit($xml,$filename);	
								 
							} else {
								echo $obj->gFormInsert('l_template',$filename);	
                    ...
                    <table boder="0" cellpadding="0" cellspacing="0" width="100%" class="generalinfo" style="margin-top:0px;">
                		...
                		<fieldset>
							...
							<input name="add" id="add" type="button" size="" value="<?=($nationid)?" save change ":" add "?>" onClick='<?=($nationid)?"set_editData(\"l_template\",$nationid)":"set_insertData(\"l_template\")"?>' > 
