...
//query language
$sql = "select l_marketingcode.* from l_marketingcode ";
	...
	for($j=0;$j<$column;$j++){
		if($arrShowinList[$j]!="no") {
				...
				if($arrFields[$j]=='category_id') {
					...
					if($chkPageEdit){
						if($rs[$i]["active"]==1){
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('l_marketingcode',".$rs[$i]["mkcode_id"].",0);\"\>".
								 "<img src=\"/images/active.png\" border=\"0\" title=\"active\" /></a>";
						}else{
							$data = "<a href=\"javascript:;\" onClick=\"setEnable('l_marketingcode',".$rs[$i]["mkcode_id"].",1);\">".
				...
				}else if($arrFields[$j]=='sign'||$arrFields[$j]=='place'||$arrFields[$j]=='contactperson'||$arrFields[$j]=='phone'){
...
<?		}
	...
	if($chkPageEdit){
		echo "<td class=\"report\"><a href=\"javascript:;\" onclick=\"editData('$tbname','".$rs[$i]["mkcode_id"]."')\">update</a></td>";
	}
	echo "</tr>";
}
if(!$rs["rows"]){}
