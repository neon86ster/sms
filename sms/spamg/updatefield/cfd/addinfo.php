        	...
        	<div id="gform">
        		        <? 
							if($mkcodeid){
									...
									"<where name='mkcode_id' operator='='>$mkcodeid</where>" .
									"</command>";
								echo $obj->gFormEdit($xml,$filename);	 
							} else {
								echo $obj->gFormInsert('l_marketingcode',$filename);	
							}
 						?>
                    <table boder="0" cellpadding="0" cellspacing="0" width="100%" class="generalinfo" style="margin-top:0px;">
                		...
                		<fieldset>
                    		...
                    		<input name="id" id="id" type="hidden" value="<?=$mkcodeid?>">
                    		<input name="add" id="add" type="button" size="" value="<?=($mkcodeid)?" save change ":" add "?>" onClick='<?=($mkcodeid)?"set_editData(\"l_marketingcode\",$mkcodeid)":"set_insertData(\"l_marketingcode\")"?>' >
