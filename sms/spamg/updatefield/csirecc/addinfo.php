...
<body>
<form name="csirecc" id="csirecc" action="" method="post" style="padding:0;margin:0">
    	...
    	<td width="100%">
        	...
        	<div id="gform">
        		        <? 
							if($rec_id){
									$xml = "<command>" .
									"<table>fl_csi_recommend</table>" .
									"<where name='rec_id' operator='='>$rec_id</where>" .
									"</command>";
								echo $obj->gFormEdit($xml,$filename);	 
							} else {
								echo $obj->gFormInsert('fl_csi_recommend',$filename);	
							}
 						?>
                    <table boder="0" cellpadding="0" cellspacing="0" width="100%" class="generalinfo" style="margin-top:0px;">
                		...
                		<fieldset>
                    		...
                    		<input name="id" id="id" type="hidden" value="<?=$rec_id?>">
							<input name="add" id="add" type="button" size="" value="<?=($rec_id)?" save change ":" add "?>" onClick='<?=($rec_id)?"set_editData(\"fl_csi_recommend\",$rec_id)":"set_insertData(\"fl_csi_recommend\")"?>' > 
