...
$querystr = "pageid=$pageid&begin=$hidden_begin&end=$hidden_end&branchid=$branch&cityid=$city&order=$order&sortby=$sortby&column=$column&Collapse=$collapse&commission=$cmschk&search='+document.getElementById('search').value.replace('+','%2B')+'";
...
<form name="accom" id="accom" action='' method='post' style="padding:0;margin:0">
    		...
    		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			      <tr>
			        <td height="30" class="rheader" style="padding-left: 20px;background-image: url('/images/<?=$theme?>/appt/menubg.png');"> Export:&nbsp;&nbsp;
			          ...
			          &nbsp;&nbsp;<input type="button" name="Export" id="Export" value="Export" onClick="window.open('report.php?begin=<?=$hidden_begin?>&end=<?=$hidden_end?>&branchid=<?=$branch?>&cityid=<?=$city?>&order=<?=$order?>&sortby=<?=$sortby?>&column=<?=$column?>&commission=<?=$cmschk?>&Collapse=<?=$collapse?>&search='+document.getElementById('search').value+'&export='+document.getElementById('export').value)"/>
			          ...
			          <!--
			          &nbsp;&nbsp;<input type="checkbox" name="commission" id="commission" value="checked" onClick="this.form.submit()" <?=$cmschk?>/> 
			          &nbsp;&nbsp;have commission
			          -->
					  </td>
			        ...
			        <td height="30" class="rheader" style="padding-left: 20px;background-image: url('/images/<?=$theme?>/appt/menubg.png');"> 
				       <input type="text" name="search" id="search" <?=($search)?"value='$search'":""?> title="Search Commissions by phone."/>
				        ...
				        &nbsp;&nbsp;<a href="javascript:;" onClick="document.accom.submit();" class="top_menu_link" title="Search by booking person,phone and customer name only.">
				        <img src="/images/btn_search_bg.gif" alt="search" width="16" height="16" border="0"/></a>&nbsp; 
				        <a href="javascript:;" onClick="document.accom.submit();">Search</a> &nbsp;
				        
				        <a href="javascript:;" onClick="document.getElementById('search').value='';getReturnText('report.php','begin=<?=$hidden_begin?>&end=<?=$hidden_end?>&branchid=<?=$branch?>&cityid=<?=$city?>&order=<?=$order?>&sortby=<?=$sortby?>&column=<?=$column?>&commission=<?=$cmschk?>&Collapse=<?=$collapse?>&search=','tableDisplay')" class="top_menu_link" title="View all customer Accomodations.">
				        <img src="/images/btn_show-all_bg.gif" alt="search" width="16" height="16" border="0"/></a>&nbsp; 
				        <a href="javascript:;" onClick="document.getElementById('search').value='';getReturnText('report.php','begin=<?=$hidden_begin?>&end=<?=$hidden_end?>&branchid=<?=$branch?>&cityid=<?=$city?>&order=<?=$order?>&sortby=<?=$sortby?>&column=<?=$column?>&commission=<?=$cmschk?>&Collapse=<?=$collapse?>&search=','tableDisplay')" class="top_menu_link" title="View all customer Accomodations.">View All</a>
				        &nbsp;&nbsp; 
					  </td>
