...
$rs = $obj->getcustnum(0,$begin_date,$end_date);
for($j=0; $j<$rs["rows"]; $j++){
	$qty[$j]=$rs[$j]["qty"];
}
...
<?	
	...
	if($a==$alltable-1){$allcolumncnt+=1;}
    	...
    	<td class="content" width="100%" align="center">
			<table cellspacing="0" border="0" cellpadding="0">
				<tr>		<!-- set column width for export to pdf 1 -->
					...
					<td width="<?=$columnwidth?>%"></td>
					<? }?>
					<? if($a==$alltable-1){?><td width="<?=$columnwidth?>%"></td><?} ?>
				</tr>
				<tr>
				...
				<tr height="35">
					...
					<? }?>
					<? if($a==$alltable-1){?><td width="40" style="text-align:right;padding-left:10px;overflow:hidden; border-top:2px #000000 solid;border-bottom:2px #ff0000 solid;"><b style="text-decoration: underline;">PERCENT</b></td><?}?>
				</tr>
<?
...
<tr height="32">
	...
	<td colspan="<?=($column=="Total only")?$rsdate["rows"]+$pr:$datechk["rows"]+$pr?>" style="padding-left:7px; white-space: nowrap; border-bottom:3px #000000 double;" bgcolor="#d3d3d3">&nbsp;</td><!-- input city -->
</tr>
<?
...
<?	} ?>
			<tr height="28">
				...
				<td colspan="<?=($column=="Total only")?$rsdate["rows"]+$pr:$datechk["rows"]+$pr?>" style="padding-left: 20px;">&nbsp;</td>
			</tr>
					<?
