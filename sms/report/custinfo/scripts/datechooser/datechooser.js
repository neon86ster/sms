...
function showChooser(obj, inputId, divId, start, end, format, isTimeChooser, isApptpage) {
    if (document.getElementById) {
        var input = document.getElementById(inputId);
        var div = document.getElementById(divId);
        if (input !== undefined && div !== undefined) {
            if (input.DateChooser === undefined) {
                input.DateChooser = new DateChooser(input, div, start, end, format, isTimeChooser, isApptpage);
            }
...
//add check report document input date on 2 Feb 2009
function dateChooserSetDate(inputId, value, date) {
	var input = document.getElementById(inputId);
	var hinput = document.getElementById('hidden_'+inputId);
	
    //for report document
    if(inputId=="begin"){
    	var hinput_end = document.getElementById('hidden_end').value;
    	if(hinput_end<date){
    		alert("please check date value begin date must less than end date!!");
    		return false;
    	}
    }
    if(inputId=="end"){
    	var hinput_begin = document.getElementById('hidden_begin').value;
    	if(hinput_begin>date){
    		alert("please check date value begin date must less than end date!!");
    		return false;
    	}
    }
	...
	if (input !== undefined && input.DateChooser !== undefined) {
        ...
        //for report document
        if(inputId=="begin"||inputId=="end"){
        	document.getElementById("date").options[1].selected = true;
        	input.form.submit();
        }
    }
}
...
function DateChooser(input, div, start, end, format, isTimeChooser, isApptpage) {
    this._input = input;
    this._div = div;
    this._start = start;
    this._end = end;
...
DateChooser.prototype.createCalendarHtml = function() {
    ...
    while (i < this._date.getDaysInMonth()) {
		...
		if(this._isApptpage){
			js += 'chkvalue('
            	+ this._input.form.elements['branch_id'].value + ',\'' + thisDay.dateFormat(this._format) + '\')'
		}
			js += ';"'
