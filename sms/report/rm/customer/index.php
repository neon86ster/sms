...
$branchid = $obj->getParameter("branchid",0,1);
...
$chkarr = array("member_code","cs_birthday","sex_id","nationality_id");
if(!in_array($order,$chkarr)){$order="b_appt_date";}
$sortby = $obj->getParameter("chksortby","Z &gt A");
$chkarr = array("Z > A","A > Z");
if(!in_array($sortby,$chkarr)){$sortby="Z &gt A";}

$memchk = $obj->getParameter("memchk");
if($memchk!="checked"){$memchk="";}

$querystr = "pageid=$pageid&begin=$hidden_begin&end=$hidden_end&branchid=$branchid&cityid=$cityid&search=$searchstr&order=$order&sortby=$sortby&memchk=$memchk";
...
<form name="custinfo" id="custinfo" action='index.php' method='post' style="padding:0;margin:0">
			...
			<table border="0" cellspacing="0" cellpadding="0">
			      <tr>
			        ...
			        <td class="rheader" height="30" align="right" style="padding-left: 20px; background-image: url('/images/<?=$theme?>/appt/menubg.png');">
				        Branch:&nbsp;&nbsp;
						<?=$obj->makeListbox("branchid","all_bl_branchinfo","branch_name","branch_id",$branchid,0,"branch_name","branch_active","1","branch_name!='All'")?>
						&nbsp;&nbsp;City:&nbsp;&nbsp;
						<?=$obj->makeListbox("cityid","all_al_city","city_name","city_id",$cityid,0,"city_name","city_active","1")?>
					</td>
		  		</tr>
    		</table>  
    	</td>
    		...
    		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			      <tr>
			        <td height="30" class="rheader" style="padding-left: 20px;background-image: url('/images/<?=$theme?>/appt/menubg.png');"> Export:&nbsp;&nbsp;
						...
						&nbsp;&nbsp;<a href="javascript:;" onClick="searchCust('<?=$cityid?>','<?=$branchid?>','');" class="top_menu_link" title="Search all customer information.">
				        <img src="/images/btn_search_bg.gif" alt="search" width="16" height="16" border="0"/></a>&nbsp; 
				        <a href="javascript:;" onClick="searchCust('<?=$cityid?>','<?=$branchid?>','');" class="top_menu_link" title="Search all customer information.">Search</a> &nbsp;
				        <a href="javascript:;" onClick="document.getElementById('search').value='';searchCust('0','0','');" class="top_menu_link" title="View all customer information.">
				        <img src="/images/btn_show-all_bg.gif" alt="search" width="16" height="16" border="0"/></a>&nbsp; 
				        <a href="javascript:;" onClick="document.getElementById('search').value='';searchCust('0','0','');" class="top_menu_link" title="View all customer information.">View All</a>
				        &nbsp;&nbsp; 
						<input type="checkbox" name="memchk" id="memchk" value="checked" <?=$memchk?>/> Exclude Member
					  </td>
			        </tr>
			        ...
			        <td class="rheader" height="30" style="padding-left: 20px;background-image: url('/images/<?=$theme?>/appt/menubg.png');">
				       ...
				       <span style="width: 115px;font-family:Tahoma; font-size: 12px;overflow:hidden;">
					    <select id="order" name="order" class="ctrDropDown" onBlur="this.className='ctrDropDown';" onMouseDown="this.className='ctrDropDownClick';" onChange="this.className='ctrDropDown';">	    
				          <option title="Appointment Date" value="b_appt_date" <?=($order=="b_appt_date")?"selected":""?>>Appointment Date</option>
				          <option title="Member Code" value="member_code" <?=($order=="member_code")?"selected":""?>>Member Code</option>
				          <option title="Birth date" value="cs_birthday" <?=($order=="cs_birthday")?"selected":""?>>Birthday (date-month)</option>
