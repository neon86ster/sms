...
$bpid = $obj->getParameter("bpid",0);
$today = date("Ymd");
$chksearch = $obj->convert_char($search);
$rs = $obj->getbookinfo($begindate,$enddate,$chksearch,$branch,$city,$bpid);
...
$reportname = " Relationship Management Detail";
if($bpid){
	$bpname = $obj->getIdToText($bpid,"al_bookparty","bp_name","bp_id");
	$reportname = $bpname."'s ".$reportname;
}else{
	if(!$branch){
		if($city){
...
if(!isset($rs[$i]["has_cms"])){$rs[$i]["has_cms"]=0;}
if($rs[$i]["has_cms"]==1){
...
?>					
			<tr <?=$bgcolor?> height="20">
					...
					<td class="report" align="center">&nbsp;<?=$bpphone?></td>
