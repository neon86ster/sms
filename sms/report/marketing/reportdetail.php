...
$querystr = "pageid=$pageid&begin=$hidden_begin&end=$hidden_end" .
		"&branchid=$branchid&cityid=$cityid&tbname=$tbname&mktype=$mktype" .
		"&status=$status&mkid=$mkcode&mktypeid=$mktypeid";
?>
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title><?=$pageinfo["pagename"]?> Detail</title>
    		...
    		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			      <tr>
			        <td height="30" class="rheader" style="padding-left: 20px;background-image: url('/images/<?=$theme?>/appt/menubg.png');"> Export:&nbsp;&nbsp;
						...
						<input type="hidden" id="mktype" name="mktype" value="<?=$mktype?>"/>
						<input type="hidden" id="pageid" name="pageid" value="<?=$pageid?>"/>
						<input type="hidden" id="status" name="status" value="<?=$status?>"/>
						<input type="hidden" id="mkid" name="mkid" value="<?=$mkcode?>"/>
						<input type="hidden" id="mktypeid" name="mktypeid" value="<?=$mktypeid?>"/>
			          </td>
			        </tr>
			      <tr bgcolor="#999999">
