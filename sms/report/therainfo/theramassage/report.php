...
$rs = $obj->getthmsginfo(0, $begin_date, $end_date, $msgid, 0, "massage", $order, $sort);
...
$reportname = "Therapist Massage Report";
?>
...
?>
						...
						<td align="right">
								<?if($export==false){?><a href="javascript:;" style="text-decoration:none; color:#000000;" onClick="openthmsg('<?=$msg["massage_id"][$i]?>','<?=$rsdate["begin"][$d]?>','<?=$rsdate["end"][$d]?>');"><? } ?>
								<? list($hr,$min) = explode(".",number_format($tmp[$i][$d],2,".",",")); ?>
								<?=$hr.".".$minsperiod[$min]?>
								<?if($export==false){?></a><? } ?>
						</td>
...
?>
						<td align="right">
								<?if($export==false){?><a href="javascript:;" style="text-decoration:none; color:#000000;" onClick="openthmsg('<?=$msg["massage_id"][$i]?>','<?=$begin_date?>','<?=$end_date?>');"><?}?>
								<? list($hr,$min) = explode(".",number_format($total[$i],2,".",",")); ?>
								<?=$hr.".".$minsperiod[$min]?>
								<?if($export==false){?></a><?}?>
						</td>
...
?>		
			<td align="right">
			<?if($export==false){?><a href="javascript:;" style="text-decoration:none; color:#000000;" onClick="openthmsg('<?=$tmpmsgtotal[$k]?>','<?=$rsdate["begin"][$d]?>','<?=$rsdate["end"][$d]?>');"><?}?>
			<?
...
?>
			<? if($column!="Total only"){ ?>
			<td align="right">
			<?if($export==false){?><a href="javascript:;" style="text-decoration:none; color:#000000;" onClick="openthmsg('<?=$tmpmsgtotal[$k]?>','<?=$begin_date?>','<?=$end_date?>');"><?}?>
			<? list($hr,$min) = explode(".",number_format($total[$k],2,".",",")); ?>
			<?=$hr.".".$minsperiod[$min]?>
			<?if($export==false){?></a><?}?>
			</td>
...
<?
	...
	for ($d = 0; $d < $rsdate["rows"]; $d++) { // start branch total loop
		...
		if ($export == false) {
			...
			echo "<a href=\"javascript:;\" style=\"text-decoration:none; color:#000000;\" onClick=\"openthmsg('" . $msg["massage_id"][$i] . "','" . $rsdate["begin"][$d] . "','" . $rsdate["end"][$d] . "');\">";
			echo $hr.".".$minsperiod[$min];
			echo "</a></b></td>\n";
		} else {
...
?>
			...
			<?if($export==false){?><a href="javascript:;" style="text-decoration:none; color:#000000;" onClick="openthmsg('<?="0"?>','<?=$begin_date?>','<?=$end_date?>');"><?}?>
			<? list($hr,$min) = explode(".",number_format($alltotal,2,".",",")); ?>
			<?=$hr.".".$minsperiod[$min]?>
			<?if($export==false){?></a><?}?>
			</b></td>
