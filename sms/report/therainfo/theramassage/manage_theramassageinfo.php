...
$msgid= $obj->getParameter("msgid");
$rs = $obj->getthmsginfo(0,$begin_date,$end_date,$msgid);
for($j=0; $j<$rs["rows"]; $j++){
	$total[$j]=$rs[$j]["total"];
}
...
$msgname = $obj->getIdToText($msgid,"db_trm","trm_name","trm_id");
if(!$msgname){$msgname = "All Massages";}
$reportname = "Therapist's $msgname Report";
...
<?
		for($j=0; $j<$rsbranch["rows"]; $j++) {		// start branch loop
				...
				<td colspan="<?=($column=="Total only")?$rsdate["rows"]:$rsdate["rows"]+1?>" style="padding-left: 20px;">&nbsp;</td>
					...
					for($k=0; $k<$rsthera["rows"]; $k++) { 	// start branch name loop
						...
						<tr height="22">
							...
							?>		
								<td align="right">
									<?if($export==false){?><a href="javascript:;" class="pagelink" style="text-decoration:none;color:#000000;" onClick="openmsgDetail(<?=$rsdate["begin"][$d].",".$rsdate["end"][$d].",".$rsthera[$k]["emp_id"].",".$rscity[$i]["city_id"].",0,'$msgid'"?>)"><? } ?>
									<?= number_format(str_replace(".5",".3",$tmp[$k][$d]),2,".",",") 	?>
									<?if($export==false){?></a><? } ?>
								</td>
								<? } ?>	
							<? 
							   if($column!="Total only"){ ?>
							<td align="right">
							<?if($export==false){?><a href="javascript:;" class="pagelink" style="text-decoration:none;color:#000000;" onClick="openmsgDetail(<?=$rsdate["begin"][0].",".$rsdate["end"][$d-1].",".$rsthera[$k]["emp_id"].",".$rscity[$i]["city_id"].",0,'$msgid'"?>)"><?}?>
							<?= number_format(str_replace(".5",".3",$total[$k]),2,".",",")?>
							<?if($export==false){?></a><?}?>
							</td>
							<? } ?>
			...
			<tr height="28">
				...
				?>		
				<td align="right" style="border-top:1px #000000 solid;">
				<?if($export==false){?><a href="javascript:;" style="text-decoration:none;color:#000000;" onClick="openmsgDetail(<?=$rsdate["begin"][$d].",".$rsdate["end"][$d].",0,".$rscity[$i]["city_id"].",".$rsbranch[$j]["branch_id"].",'$msgid'"?>)"><?}?>
				<?=number_format(str_replace(".5",".3",$bttotal[$i][$j][$d]),2,".",",")?>
				<?if($export==false){?></a><?}?>
				</td>
				<? }  ?>	
				<? if($column!="Total only"){ ?>
				<td align="right" style="border-top:1px #000000 solid;">
				<?if($export==false){?><a href="javascript:;" style="text-decoration:none;color:#000000;" onClick="openmsgDetail(<?=$rsdate["begin"][0].",".$rsdate["end"][$d-1].",0,".$rscity[$i]["city_id"].",".$rsbranch[$j]["branch_id"].",'$msgid'"?>)"><?}?>
				<?=number_format(str_replace(".5",".3",$allbttotal[$i][$j]),2,".",",")?>
				<?if($export==false){?></a><?}?>
				</td>
				<? } ?>
		...
		<tr height="35">
			...
			<td align="right">
			<?if($export==false){?><a href="javascript:;" style="text-decoration:none;color:#000000;" onClick="openmsgDetail(<?=$rsdate["begin"][0].",".$rsdate["end"][$d-1].",0,".$rscity[$i]["city_id"].",0,'$msgid'"?>)"><?}?>
			<b><?=number_format(str_replace(".5",".3",$allltotal[$i]),2,".",",")?></b>
			<?if($export==false){?></a><?}?>
			</td>
		</tr>
...
?>		
			<td align="right">
			<?if($export==false){?><a href="javascript:;" class="pagelink" style="text-decoration:none;color:#000000;" onClick="openmsgDetail(<?=$rsdate["begin"][$d].",".$rsdate["end"][$d].",".$rsthera[$k]["emp_id"].",0,0,'$msgid'"?>)"><?}?>
			<?=number_format(str_replace(".5",".3",$tmp[$k][$d]),2,".",",") 	?>
			<?if($export==false){?></a><?}?>
			</td>
<?
...
?>
			<? if($column!="Total only"){ ?>
			<td align="right">
			<?if($export==false){?><a href="javascript:;" class="pagelink" style="text-decoration:none;color:#000000;" onClick="openmsgDetail(<?=$rsdate["begin"][0].",".$rsdate["end"][$d-1].",".$rsthera[$k]["emp_id"].",0,0,'$msgid'"?>)"><?}?>
			<?= number_format(str_replace(".5",".3",$total[$k]),2,".",",")?>
			<?if($export==false){?></a><?}?>
			</td>
			<? } ?>
...
?>		
			<td align="right">
			<?if($export==false){?><a href="javascript:;" class="pagelink" style="text-decoration:none;color:#000000;" onClick="openmsgDetail(<?=$rsdate["begin"][$d].",".$rsdate["end"][$d].",".$tmpemptotal[$k].",0,0,'$msgid'"?>)"><?}?>
			<?	$tmp[$k][$d]=$obj->sumeachempfield($rs,"total",$tmpemptotal[$k],$rsdate["begin"][$d],$rsdate["end"][$d]);
				echo number_format(str_replace(".5",".3",$tmp[$k][$d]),2,".",",");	?>
			<?if($export==false){?></a><?}?>
			</td>
...
?>
			<? if($column!="Total only"){ ?>
			<td align="right">
			<?if($export==false){?><a href="javascript:;" class="pagelink" style="text-decoration:none;color:#000000;" onClick="openmsgDetail(<?=$rsdate["begin"][0].",".$rsdate["end"][$d-1].",".$tmpemptotal[$k].",0,0,'$msgid'"?>)"><?}?>
			<?= number_format(str_replace(".5",".3",$total[$k]),2,".",",")?>
			<?if($export==false){?></a><?}?>
			</td>
			<? } ?>
...
for($d=0;$d<$rsdate["rows"];$d++){ // start branch total loop
		echo "<td style=\"padding-left: 20px; white-space: nowrap;\" align=\"right\" bgcolor=\"#d3d3d3\"><b>";
		if($export==false){echo "<a href=\"javascript:;\" style=\"text-decoration:none;color:#000000;\" onClick=\"openmsgDetail(".$rsdate["begin"][$d].",".$rsdate["end"][$d].",0,0,0,'$msgid')\">".number_format(str_replace(".5",".3",$alldatetotal[$d]),2,".",",")."</a></b></td>";}
...
?>
			...
			<td align="right" bgcolor="#d3d3d3"><b>
			<?if($export==false){?><a href="javascript:;" style="text-decoration:none;color:#000000;" onClick="openmsgDetail(<?=$rsdate["begin"][0].",".$rsdate["end"][$d-1].",0,0,0,'$msgid'"?>)"><?}?>
			<?=number_format(str_replace(".5",".3",$alltotal),2,".",",")?>
