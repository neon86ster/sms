...
if($export=="Excel" && $chkPageView){
	...
	header("Content-Disposition: attachment; filename=\"Therapist Massage Detail.xls\"");
...
$rs=$obj->getthmsgdetail($begin_date,$end_date,$empid,$msgid,$branchid,$city);
$rsthera = $obj->gettherapist($order,$sort,$empid,0,$city);
$rsbranch = $obj->getbranch($order,$sort,$branchid,$city);
$reportname = "Therapist Massage Detail";
...
?>
					...
					<?if(!$export){?><a href='javascript:;;'  style="color:#000000;"onClick="newwindow('/appt/<?=$url?>','<?=$pagename?>')" class="menu"><?}?>
					<?=$bpdsid?>
					<?if(!$export){?></a><?}?>
					</td>
