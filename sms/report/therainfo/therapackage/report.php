...
$rs = $obj->getthpackageinfo(0, $begin_date, $end_date, $packageid, 0, "package", $order, $sort);
...
$reportname = "Therapist Package Report";
?>
...
?>
						...
						<td align="right">
								<?if($export==false){?><a href="javascript:;" style="text-decoration:none; color:#000000;" onClick="openthpackage('<?=$package["package_id"][$i]?>','<?=$rsdate["begin"][$d]?>','<?=$rsdate["end"][$d]?>');"><? } ?>
								<?=number_format(str_replace(".5",".3",$tmp[$i][$d]),2,".",",")?>
								<?if($export==false){?></a><? } ?>
						</td>
					<?
...
?>
									<td align="right">
									<?if($export==false){?><a href="javascript:;" style="text-decoration:none; color:#000000;" onClick="openthpackage('<?=$package["package_id"][$i]?>','<?=$begin_date?>','<?=$end_date?>');"><?}?>
									<?=number_format(str_replace(".5",".3",$total[$i]),2,".",",")?>
									<?if($export==false){?></a><?}?>
									</td>
					<?
...
?>		
			<td align="right">
			<?if($export==false){?><a href="javascript:;" style="text-decoration:none; color:#000000;" onClick="openthpackage('<?=$tmppackagetotal[$i]?>','<?=$rsdate["begin"][$d]?>','<?=$rsdate["end"][$d]?>');"><?}?>
			<?
...
?>
			<? if($column!="Total only"){ ?>
			<td align="right">
			<?if($export==false){?><a href="javascript:;" style="text-decoration:none;color:#000000;" onClick="openthpackage('<?=$tmppackagetotal[$i]?>','<?=$begin_date?>','<?=$end_date?>');"><?}?>
			<?=number_format(str_replace(".5",".3",$total[$i]),2,".",",")?>
			<?if($export==false){?></a><?}?>
			</td>
			<? } ?>
...
<?
	...
	for ($d = 0; $d < $rsdate["rows"]; $d++) { // start branch total loop
		...
		if ($export == false) {
			echo "<a href=\"javascript:;\" style=\"text-decoration:none;color:#000000;\" onClick=\"openthpackage('','" . $rsdate["begin"][$d] . "','" . $rsdate["end"][$d] . "');\">";
			echo number_format(str_replace(".5", ".3", $alldatetotal[$d]), 2, ".", ",");
			echo "</a></b></td>\n";
		} else {
...
?>
			...
			<?if($export==false){?><a href="javascript:;" style="text-decoration:none;color:#000000;" onClick="openthpackage('<?="0"?>','<?=$begin_date?>','<?=$end_date?>');"><?}?>
			<?=number_format(str_replace(".5",".3",$alltotal),2,".",",")?>
