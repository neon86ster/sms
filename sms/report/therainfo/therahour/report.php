...
$rs = $obj->getthhourinfo(0,$begin_date,$end_date,$empid);
for($j=0; $j<$rs["rows"]; $j++){
	$total[$j]=$rs[$j]["total"];
}
...
if($export=="Excel" && $chkPageView){
	...
	header("Content-Disposition: attachment; filename=\"Therapist Hour Report.xls\"");
...
$reportname = "Therapist Hours Report";
...
?>
		<td align="right"><?= $hr.".".$minsperiod[$min]?></td>
							<? } ?>
</tr><?if($rowcnt%$chkrow==0){echo $header;} ?>
<?			}
		} 
...
<?
		for($j=0; $j<$rsbranch["rows"]; $j++) {		// start branch loop
			...
			<tr height="28">
				...
				<td colspan="<?=($column=="Total only")?$rsdate["rows"]:$rsdate["rows"]+1?>" style="padding-left: 20px;">&nbsp;</td>
					...
					<?
					for($k=0; $k<$rsthera["rows"]; $k++) { 	// start therapist name loop
						if($rsthera[$k]["branch_id"]==$rsbranch[$j]["branch_id"]&&$rsbranch[$j]["city_id"]==$rscity[$i]["city_id"]
						&& $total[$k]>0){
						?>
						<tr height="22">
							...
							?>		
								<td align="right">
									<?if($export==false){?><a href="javascript:;" class="pagelink" style="text-decoration:none; color:#000000;" onClick="openDetail(<?=$rsdate["begin"][$d].",".$rsdate["end"][$d].",".$rsthera[$k]["emp_id"].",".$rscity[$i]["city_id"].",0"?>)"><? } ?>
									<? list($hr,$min) = explode(".",number_format($tmp[$k][$d],2,".",",")); ?>
									<?=$hr.".".$minsperiod[$min]?>
									<?if($export==false){?></a><? } ?>
								</td>
							...
							<? 
							   if($column!="Total only"){ ?>
							<td align="right">
							<?if($export==false){?><a href="javascript:;" class="pagelink" style="text-decoration:none; color:#000000;" onClick="openDetail(<?=$rsdate["begin"][0].",".$rsdate["end"][$d-1].",".$rsthera[$k]["emp_id"].",".$rscity[$i]["city_id"].",0"?>)"><?}?>
							<? list($hr,$min) = explode(".",number_format($total[$k],2,".",",")); ?>
							<?=$hr.".".$minsperiod[$min]?>
							<?if($export==false){?></a><?}?>
							</td>
			...
			<tr height="28">
				...
				?>		
				<td align="right" style="border-top:1px #000000 solid;">
				<?if($export==false){?><a href="javascript:;" class="pagelink" style="text-decoration:none; color:#000000;" onClick="openDetail(<?=$rsdate["begin"][$d].",".$rsdate["end"][$d].",0,".$rscity[$i]["city_id"].",".$rsbranch[$j]["branch_id"]?>)"><?}?>
				<? list($hr,$min) = explode(".",number_format($bttotal[$i][$j][$d],2,".",",")); ?>
				<?=$hr.".".$minsperiod[$min]?>
				<?if($export==false){?></a><?}?>
				</td>
				...
				<? if($column!="Total only"){ ?>
				<td align="right" style="border-top:1px #000000 solid;">
				<?if($export==false){?><a href="javascript:;" class="pagelink" style="text-decoration:none;color:#000000;" onClick="openDetail(<?=$rsdate["begin"][0].",".$rsdate["end"][$d-1].",0,".$rscity[$i]["city_id"].",".$rsbranch[$j]["branch_id"]?>)"><?}?>
				<? list($hr,$min) = explode(".",number_format($allbttotal[$i][$j],2,".",",")); ?>
				<?=$hr.".".$minsperiod[$min]?>
				<?if($export==false){?></a><?}?>
				</td>
		...
		<tr height="35">
			...
			<td align="right">
			<?if($export==false){?><a href="javascript:;" class="pagelink" style="text-decoration:none;color:#000000;" onClick="openDetail(<?=$rsdate["begin"][0].",".$rsdate["end"][$d-1].",0,".$rscity[$i]["city_id"].",0"?>)"><?}?>
			<? list($hr,$min) = explode(".",number_format($allltotal[$i],2,".",",")); ?>
			<b><?=$hr.".".$minsperiod[$min]?></b>
			<?if($export==false){?></a><?}?>
			</td>
...
?>		
			<td align="right">
			<?if($export==false){?><a href="javascript:;" class="pagelink" style="text-decoration:none;color:#000000;" onClick="openDetail(<?=$rsdate["begin"][$d].",".$rsdate["end"][$d].",".$rsthera[$k]["emp_id"].",0,0"?>)"><?}?>
			<? list($hr,$min) = explode(".",number_format($tmp[$k][$d],2,".",",")); ?>
			<?=$hr.".".$minsperiod[$min]?>
			<?if($export==false){?></a><?}?>
			</td>
...
?>
			<? if($column!="Total only"){ ?>
			<td align="right">
			<?if($export==false){?><a href="javascript:;" class="pagelink" style="text-decoration:none;color:#000000;" onClick="openDetail(<?=$rsdate["begin"][0].",".$rsdate["end"][$d-1].",".$rsthera[$k]["emp_id"].",0,0"?>)"><?}?>
			<? list($hr,$min) = explode(".",number_format($total[$k],2,".",",")); ?>
			<?=$hr.".".$minsperiod[$min]?>
			<?if($export==false){?></a><?}?>
			</td>
...
?>		
			<td align="right">
			<?if($export==false){?><a href="javascript:;" class="pagelink" style="text-decoration:none;color:#000000;" onClick="openDetail(<?=$rsdate["begin"][$d].",".$rsdate["end"][$d].",".$tmpemptotal[$k][0].",0,0"?>)"><?}?>
...
?>
			<? if($column!="Total only"){ ?>
			<td align="right">
			<?if($export==false){?><a href="javascript:;" class="pagelink" style="text-decoration:none;color:#000000;" onClick="openDetail(<?=$rsdate["begin"][0].",".$rsdate["end"][$d-1].",".$tmpemptotal[$k][0].",0,0"?>)"><?}?>
			<? list($hr,$min) = explode(".",number_format($total[$k],2,".",",")); ?>
			<?=$hr.".".$minsperiod[$min]?>
			<?if($export==false){?></a><?}?>
			</td>
...
for($d=0;$d<$rsdate["rows"];$d++){ // start branch total loop
		...
		list($hr,$min) = explode(".",number_format($alldatetotal[$d],2,".",","));
		if($export==false){
			echo "<a href=\"javascript:;\" class=\"pagelink\" style=\"text-decoration:none;color:#000000;\" onClick=\"openDetail(".$rsdate["begin"][$d].",".$rsdate["end"][$d].",0,0,0)\">".$hr.".".$minsperiod[$min]."</a></b></td>";
		}else{
			echo $hr.".".$minsperiod[$min]."</a></b></td>";
		}
}
...
?>
			...
			<td align="right" bgcolor="#d3d3d3"><b>
			<?if($export==false){?><a href="javascript:;" class="pagelink" style="text-decoration:none;color:#000000;" onClick="openDetail(<?=$rsdate["begin"][0].",".$rsdate["end"][$d-1].",0,0,0"?>)"><?}?>
				<? list($hr,$min) = explode(".",number_format($alltotal,2,".",",")); ?>
				<?=$hr.".".$minsperiod[$min]?>
