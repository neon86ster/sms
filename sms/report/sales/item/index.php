...
$acc_func = $obj->getParameter("acc_func",2,1);
$querystr = "pageid=$pageid&begin=$hidden_begin&end=$hidden_end&branchid=$branch&cityid=$cityid&order=$order&sortby=$sortby&acc_func=$acc_func";
...
<!--[if IE]>
<style>
td.rheader span.fix select.ctrDropDown{
    width:115px;
    font-size:12px;
}
td.rheader span.fix select.ctrDropDownClick{
    font-size:12px;

    width:auto;

}
td.rheader span.fix select.plainDropDown{
    width:115px;
    font-size:12px;
}
</style>
...
<form name="item" id="item" action='index.php' method='post' style="padding:0;margin:0">
    	...
    	<td valign="top" height="30px" colspan="2" class="rheader" style="background-image: url('/images/<?=$theme?>/appt/menubg.png');">
			<table border="0" cellspacing="0" cellpadding="0">
			      <tr>
			        ...
			        <span class="fix" style="width: 115px;font-family:Tahoma; font-size: 12px;overflow:hidden;">
    		...
    		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			      <tr>
			        <td height="30" class="rheader" style="padding-left: 20px;background-image: url('/images/<?=$theme?>/appt/menubg.png');"> Export:&nbsp;&nbsp;
          			...
          			&nbsp;&nbsp;Sales Items:
				    <span class="fix" style="width: 115px;font-family:Tahoma; font-size: 12px;overflow:hidden;">
					    <select id="acc_func" name="acc_func" class="ctrDropDown" onBlur="this.className='ctrDropDown';" onMouseDown="this.className='ctrDropDownClick';" onChange="this.className='ctrDropDown';">	    
				          <option title="All Sales" value="0" <?=($acc_func=="0")?"selected":""?>>All Sales</option>
				          <option title="Discount Report" value="1" <?=($acc_func=="1")?"selected":""?>>Discount Report</option>
				          <option title="Non-Discount Report" value="2" <?=($acc_func=="2")?"selected":""?>>Before Discount Report</option>
				        </select>
					</span>	
					...
					&nbsp;&nbsp;Sort by:
				        <select id="order" name="order">
				          ...
				          <option title="Total Amount" value="Amount" <?=($order=="Amount")?"selected":""?>>Total Amount</option>
				          <option title="Total Qty" value="Qty" <?=($order=="Qty")?"selected":""?>>Total Qty</option>
				         </select>
