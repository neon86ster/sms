...
$order = $obj->getParameter("order","Category");
$sortby = $obj->getParameter("chksortby","Z &gt A");
$querystr = "pageid=$pageid&begin=$hidden_begin&end=$hidden_end&branchid=$branch&cityid=$city&Collapse=$collapse&commission=$cmschk&order=$order&sortby='+document.getElementById('sort').value+'";
}else{
$querystr = "pageid=$pageid&begin=$hidden_begin&end=$hidden_end&branchid=$branch&cityid=$city&Collapse=$collapse&commission=$cmschk";
}
			      ...
			      <tr>
			        ...
			        <td class="rheader" height="30" align="right" style="padding-left: 20px; background-image: url('/images/<?=$theme?>/appt/menubg.png');">
			         ...
			         <select id="order" name="order">
			          ...
			          <option title="Total Bookings" value="TotalBookings" <?=($order=="TotalBookings")?"selected":""?>>Total Bookings</option>
			          <option title="Total Customers" value="TotalCustomers" <?=($order=="TotalCustomers")?"selected":""?>>Total Customers</option>
			          <option title="Total Sales" value="TotalSales" <?=($order=="TotalSales")?"selected":""?>>Total Sales</option>
			         </select>
    		...
    		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			      <tr>
			        <td height="30" class="rheader" style="padding-left: 20px;background-image: url('/images/<?=$theme?>/appt/menubg.png');"> Export:&nbsp;&nbsp;
			          ...
			          &nbsp;&nbsp;<input type="button" name="Export" id="Export" value="Export" onClick="window.open('report.php?begin=<?=$hidden_begin?>&end=<?=$hidden_end?>&branchid=<?=$branch?>&commission=<?=$cmschk?>&Collapse=<?=$collapse?>&export='+document.getElementById('export').value)"/>
