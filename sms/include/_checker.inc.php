 ...
 */
	function getcrs($branch_id=false,$start_date=false,$end_date=false,$debug=false){
		if(!$start_date) {
		...
		$sql1 .= "from c_salesreceipt left join " .
				"(SELECT log_c_srprint.salesreceipt_id, log_c_srprint.reprint_times, log_c_srprint.l_lu_user 
					FROM log_c_srprint
 ...
 */	
	function getttcs($branch_id=false,$start_date=false,$end_date=false,$debug=false){
		if(!$start_date) {
 ...
 */	
	function gettthour($branch_id=false,$start_date=false,$end_date=false,$debug=false){
		if(!$start_date) {
 ...
 */		
	function getcms($branch_id=false,$start_date=false,$end_date=false,$book_id=false,$debug=false){
		if(!$start_date) {
 ...
 */		
	function getbankcms($branch_id=false,$start_date=false,$end_date=false,$book_id=false,$debug=false){
		if(!$start_date) {
			$this->setErrorMsg("checker.getcms(),Please insert Date for see this report!!");
			return false;
		}
