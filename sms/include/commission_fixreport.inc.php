 ...
 */		
	function getcms($branch_id=false,$start_date=false,$end_date=false,$havecms=false,$collapse=false,$book_id=false,$cityid=false,$order=false,$sort=false,$debug=false){
		...
		if($collapse=="Expand"){
			...
			if($order){
				if($order=="TotalBookings"){
				$sql .= "order by cntbook $sortby";
				}elseif($order=="TotalCustomers"){
				$sql .= "order by qty_pp $sortby";
				}elseif($order=="TotalSales"){
				$sql .= "order by sr_total $sortby";
				}else{
				$sql .= "order by a_appointment.bp_name $sortby,a_bookinginfo.b_appt_date desc,a_bookinginfo.book_id ";
				}
			}
		...
		//$sql .= "order by a_appointment.bp_name,a_bookinginfo.b_appt_date desc,a_bookinginfo.book_id ";
