...
class appt extends cms {
	...
	*/
	...
	function makeListbox($sname = false, $tbname = false, $fieldname = false, $fieldid = false, $selected = false, $chkautosubmit = false, $order = false, $wherename = false, $wherechk = false, $andparam = false, $orparam = false, $disabled = false, $notEditBook = false, $bookBranchId = false, $javascript = false) {
			...
			if ($tbname == "l_hour") {
				...
				if ($tbname == "cl_product_category,cl_product") {
					...
					if ($tbname == "l_marketingcode,l_mkcode_category") {
						...
						} else if ($tbname == "fl_csi_value"){
							$sql = "select $fieldid,$fieldname,IF(csiv_id = '1', 0, 1) as order_state from $tbname";
						} else {
							$sql = "select $fieldid,$fieldname from $tbname";
						}
		...
		echo "<select id=\"$name\" name=\"$name\"class=\"ctrDropDown\" onBlur=this.className='ctrDropDown'; onMouseDown=this.className='ctrDropDownClick'; ";
	
		if($fieldname=="pd_name"){
		...
		if (isset ($chkautosubmit) && $chkautosubmit != false) {
			if ($tbname == "cl_product_category,cl_product") {
				echo " onChange=\"addSrd(" . $chkautosubmit[0] . "," . $chkautosubmit[1] . ",1);onChange=this.className='ctrDropDown';this.form.submit();\">";
			} else {
				echo " onChange=\"onChange=this.className='ctrDropDown';this.form.submit();\">";
			}
		} else {
			echo ">";
		}
	...
	function makeTimeListbox($sname = false, $tbname = false, $fieldname = false, $fieldid = false, $selected = false, $chkautosubmit = false, $order = false, $wherename = false, $wherechk = false, $andparam = false, $orparam = false, $disabled = false, $notEditBook = false, $bookBranchId = false, $javascript = false) {
		$limitBranch = false;
		if (!$tbname) {
			$this->setErrorMsg("Please insert table name to create list box!!");
			return false;
		...
		echo "<select style='width: 53px' id=\"$name\" name=\"$name\"class=\"ctrDropDown\" onBlur=this.className='ctrDropDown'; onMouseDown=this.className='ctrDropDownClick'; ";
	...
	function getsNet($price = false, $product = false, $j) {
		if($product["set_tax"][$j]){
			$price = ($price*100)/(100+$product["taxpercent"][$j]);
		}
		if($product["set_sc"][$j]){
			$price = ($price*100)/(100+$product["servicescharge"][$j]);
		}
	...
	function makeTherapistlistBook($sname = false, $selected = false, $chkautosubmit = false, $order = false, $andparam = false) {
		$sql = "select * from l_employee left join bl_branchinfo on l_employee.branch_id=bl_branchinfo.branch_id where l_employee.emp_active=1 and l_employee.emp_id!=1 and l_employee.emp_department_id=4 ";
		...
		for ($i = 0; $i < $row["rows"]; $i++) {
			...
			if ($row[$i]["branch_id"] && $row[$i]["emp_id"] != 1)
				echo $row[$i]["emp_nickname"] . " " . $row[$i]["emp_code"] . " " . $b_code . "</option>";
			else
				if ($row[$i]["emp_id"] == 1)
					echo " " . $row[$i]["emp_nickname"] . "</option>";
				else
					echo $row[$i]["emp_nickname"] . " " . $b_code . "</option>";
		}
		echo "</select>";
	...
	function add($cs = false, $cancel = false, $cms = false, $trf = false, $debug = false) {
		...
		if(!is_numeric($cs["cms_value"])){
			$this->setErrorMsg("Please check Commission Value is number !!");
			return false;	
		}
		if (!$csname) {
		...
		"c_bp_id,c_bp_person,c_bp_phone,c_set_cms,c_pcms_id,c_cms_value," .
		...
		"$bcompany,\"$bpname\",\"$bpphone\",$cms,$bcms_id,$bcms_value," .
	...
	function edit($cs = false, $bookid = false, $cancel = false, $cms = false, $trf = false, $debug = false) {
		...
		"c_bp_id=$bcompany,c_bp_person=\"$bpname\",c_bp_phone=\"$bpphone\",c_set_cms=$cms,c_pcms_id=$bcms_id,c_cms_value=$bcms_value," .
	...
	function addIndivi($tw = false, $bookid = false, $status = false, $debug = false) {
		...
		if (!isset($tw["cusstate"])) {
			$tw["cusstate"] = "";
		}
		if (!isset($tw["resident"])) {
			$tw["resident"] = "";
		}
		if (!isset($tw["room"])) {
			$tw["room"] = "";
		...
		if ($status == "add") {
			$sql = "insert into d_indivi_info(book_id,cs_name,cs_phone,cs_email,cs_age,cs_birthday,member_use," .
			...
			$sql .= "values($bookid,\"$csname\",\"$csphone\",\"$csemail\",\"$csage\",\"$csbirthday\",$memberuse," .
	...
	function delIndivi($bookid = false, $condition = false, $debug = false) {
	    ...
	    $chkth_sql = "select da_mult_th.book_id,da_mult_th.indivi_id from da_mult_th " .
					"where da_mult_th.book_id=$bookid $condition";
		$chk_th = $this->getResult($chkth_sql);
		

	    for($i=0;$i<$chk_th["rows"];$i++){
			$chkthsql = "select da_mult_th.book_id,da_mult_th.indivi_id,da_mult_th.therapist_id,da_mult_th.hour_id, " .
					"d_indivi_info.book_id,d_indivi_info.cs_name,d_indivi_info.room_id,d_indivi_info.package_id from da_mult_th,d_indivi_info " .
					"where d_indivi_info.indivi_id=".$chk_th[$i]["indivi_id"]."" .
					" and d_indivi_info.indivi_id = da_mult_th.indivi_id ";
		
			$chkth = $this->getResult($chkthsql);
				
		/////
		$sqlth = "insert into log_c_therapist(book_id,indivi_id,cs_name,th_id,th_hour,room,package,l_lu_user,l_lu_ip,l_lu_date,active,log_th_status) " .
				"values(".$chkth[0]["book_id"].",".$chk_th[0]["indivi_id"].",'".$chkth[0]["cs_name"]."',".$chkth[0]["therapist_id"].",".$chkth[0]["hour_id"]."," .
				"".$chkth[0]["room_id"].",".$chkth[0]["package_id"].",$userid,\"$ip\",now(),'1','3')";
		$rsInsert = $this->setResult($sqlth);
	 ...
	 */
	function addTh($tw = false, $indiviid = false, $bookid = false, $status = false, $info = false, $debug = false) {
		if (!$status) {
		...
		$thid = $tw["name"]; // int
		$hour = $tw["hour"]; // int
		$t_start_id = $tw["start_id"];
		$t_end_id = $tw["end_id"];
		
		$sql = "insert into da_mult_th(book_id,indivi_id,therapist_id,hour_id,start_id,end_id)" .
		" values($bookid,$indiviid,$thid,$hour,$t_start_id,$t_end_id)";
		
		$multhid = $this->setResult($sql);
		
		$chkth_sql = "select da_mult_th.book_id,da_mult_th.indivi_id,da_mult_th.therapist_id,da_mult_th.hour_id, " .
					"d_indivi_info.book_id,d_indivi_info.cs_name,d_indivi_info.room_id,d_indivi_info.package_id from da_mult_th,d_indivi_info " .
					"where d_indivi_info.indivi_id=".$indiviid."" .
					" and d_indivi_info.indivi_id = da_mult_th.indivi_id ";
		$chk_th = $this->getResult($chkth_sql);	
		...
		$chk_sql = "select multh_id from da_mult_th where book_id=$bookid group by indivi_id";
		$chk_rs = $this->getResult($chk_sql);
		...
		if ($chk_rs["rows"] != 1 && $chkrs["rows"] == 1) {
				$sqlth = "insert into log_c_therapist(book_id,indivi_id,cs_name,th_id,th_hour,room,package,l_lu_user,l_lu_ip,l_lu_date,active,log_th_status) " .
				"values(".$chk_th[0]["book_id"].",".$chk_th[0]["indivi_id"].",'".$chk_th[0]["cs_name"]."',".$chk_th[0]["therapist_id"].",".$chk_th[0]["hour_id"]."," .
				"".$chk_th[0]["room_id"].",".$chk_th[0]["package_id"].",$userid,\"$ip\",now(),'1','4')";
					
				$rsInsert = $this->setResult($sqlth);
		}else if($chk_rs["rows"] == 1 && $chkrs["rows"] == 1){
				$ip = $_SERVER["REMOTE_ADDR"];
	    		$userid = $this->getUserIdLogin();
				
				$sqlth = "insert into log_c_therapist(book_id,indivi_id,cs_name,th_id,th_hour,room,package,l_lu_user,l_lu_ip,l_lu_date,active,log_th_status) " .
				"values(".$chk_th[0]["book_id"].",".$chk_th[0]["indivi_id"].",'".$chk_th[0]["cs_name"]."',".$chk_th[0]["therapist_id"].",".$chk_th[0]["hour_id"]."," .
				"".$chk_th[0]["room_id"].",".$chk_th[0]["package_id"].",$userid,\"$ip\",now(),'1','0')";
		...
		##########  INSERT ONTO Log_c_therapist  Add ###########
		$ip = $_SERVER["REMOTE_ADDR"];
		$userid = $this->getUserIdLogin();
		$csname = htmlspecialchars($info["csnameinroom"]); // text
		$room = $info["room"];
		$package = $info["package"];
		
		
		$sqlth = "insert into log_c_therapist(book_id,indivi_id,cs_name,th_id,th_hour,room,package,l_lu_user,l_lu_ip,l_lu_date,active,log_th_status)" .
				" values($bookid,\"$indiviid\",\"$csname\",\"$thid\",\"$hour\",\"$room\",\"$package\",$userid,\"$ip\",now(),'1','0')";
		$this->setResult($sqlth);
		#################################################################
		...
		##########  INSERT ONTO Log_c_therapist  Del ###########
		$ip = $_SERVER["REMOTE_ADDR"];
		$userid = $this->getUserIdLogin();
		$csname = htmlspecialchars($info["csnameinroom"]); // text
		$room = $info["room"];
		$package = $info["package"];
			...
			if($thid == 1){ 
				$sqlth = "insert into log_c_therapist(book_id,indivi_id,cs_name,th_id,th_hour,room,package,l_lu_user,l_lu_ip,l_lu_date,active,log_th_status)" .
				" values($bookid,\"$indiviid\",\"$csname\",\"$thid\",\"$hour\",\"$room\",\"$package\",$userid,\"$ip\",now(),'1','2')";
				$this->setResult($sqlth);
			}
		#################################################################	
	 ...
	 * 	$chksql = "delete from da_mult_th where indivi_id=$indiviid and therapist_id=49";	
	 */
	function editTh($tw = false, $multh_id = false, $indiviid = false, $bookid = false, $info = false, $debug = false) {
		if (!$bookid) {
			$this->setErrorMsg("Please check booking id before insert to d_indivi_info!!");
			return false;
		}
		...
		$t_start_id = $tw["start_id"];
		$t_end_id = $tw["end_id"];
		##########  INSERT ONTO Log_c_therapist  Edit ###########
		$ip = $_SERVER["REMOTE_ADDR"];
		$userid = $this->getUserIdLogin();
		$csname = htmlspecialchars($info["csnameinroom"]); // text
		$room = $info["room"];
		...
		// query old therapist
		$chkthsql = "select therapist_id from da_mult_th where multh_id=$multh_id";
		$chkth = $this->getResult($chkthsql);
		
		
		// insert old therapist
		if($thid==1){ //if --select--  insert old therapist before delete
		$sqlth = "insert into log_c_therapist(book_id,indivi_id,cs_name,th_id,th_hour,room,package,l_lu_user,l_lu_ip,l_lu_date,active,log_th_status) " .
				"values($bookid,\"$indiviid\",\"$csname\",".$chkth[0]["therapist_id"].",\"$hour\",\"$room\",\"$package\",$userid,\"$ip\",now(),'1','2')";
		$this->setResult($sqlth);
		}
		 
		$sql = "update da_mult_th set book_id=$bookid,indivi_id=$indiviid,therapist_id=$thid,hour_id=$hour,start_id=$t_start_id,end_id=$t_end_id" .
		...
		if($thid != 1){ 
		$sqlth = "insert into log_c_therapist(book_id,indivi_id,cs_name,th_id,th_hour,room,package,l_lu_user,l_lu_ip,l_lu_date,active,log_th_status) " .
				"values($bookid,\"$indiviid\",\"$csname\",\"$thid\",\"$hour\",\"$room\",\"$package\",$userid,\"$ip\",now(),'1','1')";
		$this->setResult($sqlth);
		}
		#################################################################

		//$chksql = "select multh_id from da_mult_th where indivi_id=$indiviid order by therapist_id desc,hour_id desc";
		$chksql = "select multh_id from da_mult_th where indivi_id=$indiviid order by multh_id asc,hour_id desc";
		$chkrs = $this->getResult($chksql);
		//echo $chkrs["rows"];
		if ($chkrs["rows"] > 1) {
	...
	*/
	...
	function addPds($cs = false, $cc = false, $servicescharge = false, $taxpercent = false, $debug = false) {
		...
		"l_lu_user,l_lu_date,l_lu_ip,tax_id,servicescharge,a_member_code,mkcode_id) ";
		$sql .= "values(\"$date\",$branchid,$cancel,$canceldate,\"$cancelcomment\"," .
		"$userid,now(),\"$ip\",$taxid,$servicescharge,$memcode,".$cs["inspection"].")";
	 ...
	 * @modified - recoding on this function 14 july 2009 by Ruck
	 */
	function insertSaleReceipt($srd, $bookid = 0, $sr_id, $pdsid = 0, $smp) {
		...
		for ($i = 0; $i < count($srd); $i++) {
			...
			for ($j = 0; $j < count($srd[$i]); $j++) {
				...
				if ($srd[$i][$j]["pd_id"] != 1) {
					...
					//For debug undefined index: By David : 09-02-2010
					...
					//if (isset ($srd[$i][$j]["pay_price"])) {
					//	$newSrd[$amountSr][$pcount]["pay_price"] = $srd[$i][$j]["pay_price"];
					//} else {
					//	$newSrd[$amountSr][$pcount]["pay_price"] = "";
					//}
				...
				//For debug undefined index: By David : 09-02-2010	
				//if (!isset ($srd[$i][$j]["paytype"])) {
				if (!isset ($srd[$i][$j]["paytype"]) || $srd[$i][0]["pd_id"]==1) {
					$srd[$i][$j]["paytype"] = 1;
				}
				if (!isset ($srd[$i][$j]["pay_price"])) {
					$srd[$i][$j]["pay_price"] = 0;
				}
				//if ($srd[$i][$j]["paytype"] != 1 && $count!=0) {
				if($srd[$i][0]["paytype"] == 1 && $srd[$i][0]["pd_id"]!=1){
					$emptyData = false;
					$newSrd[$amountSr][0]["mpd_id"] = $srd[$i][0]["mpd_id"];
					$newSrd[$amountSr][0]["paytype"] = $srd[$i][0]["paytype"];
					$newSrd[$amountSr][$pcount]["pay_price"] = $srd[$i][$j]["pay_price"];
					$newSrd[$amountSr][0]["maxpaid"] = $srd[$i][0]["maxpaid"];
				}
				if ($srd[$i][$j]["paytype"] != 1) {
					$emptyData = false;
					$newSrd[$amountSr][$pcount]["mpd_id"] = $srd[$i][$j]["mpd_id"];
					$newSrd[$amountSr][$pcount]["paytype"] = $srd[$i][$j]["paytype"];
					$newSrd[$amountSr][$pcount]["pay_price"] = $srd[$i][$j]["pay_price"];
					$newSrd[$amountSr][0]["maxpaid"] = $srd[$i][0]["maxpaid"];
					$pcount++;
				}
	...
	function getCurrentRowsInTab($srd, $srcount) {
		...
		for ($i = 0; $i < $srcount; $i++) {
			...
			if (isset ($srd[$i])) { // For debug Undefined offset:  0,1. By Ruck : 15-05-2009
				for ($j = 0; $j < count($srd[$i]); $j++) {
					if(!isset($srd[$i][$j]["pd_id"])){$srd[$i][$j]["pd_id"]=0;}
	 ...
	 /* get current rows in sale receipt when initial Method of Payment
	 * @modified - add this function on 08 dec 2010
	 * This function will return amount of rows in each Method of Payment
	 */
	function getCurrentRowsInPay($srd, $srcount) {
		$newMpdCount = array ();
		for ($i = 0; $i < $srcount; $i++) {
			$rows = 1; 
			if (isset ($srd[$i])) { 
				for ($j = 0; $j < count($srd[$i]); $j++) {
					if(!isset($srd[$i][0]["pd_id"])){$srd[$i][0]["pd_id"]=1;}
					if (!isset ($srd[$i][$j]["paytype"]) || $srd[$i][0]["pd_id"]==1) {
					$srd[$i][$j]["paytype"] = 1;
					}
					
					if ($srd[$i][$j]["paytype"] != 1 && $srd[$i][$j]["paytype"] != 0) {
						$rows++;
					}
				}
			}
	...
	*/
	function getSaleReceiptData($bookid = 0, $pdsid = 0) {
		...
		$sqlMp = "select * from c_srpayment where book_id=$bookid and pds_id=$pdsid group by salesreceipt_id";
		$mpId = $this->getResult($sqlMp);
		//echo $sqlMp."<br>".$mpId["rows"]."<br>";
	if($mpId){
		for ($i = 0; $i < $mpId["rows"]; $i++) {
			$newSrd[$i][0]["mpd_id"] = $mpId[$i]["srpayment_id"];
			$newSrd[$i][0]["paytype"] = $mpId[$i]["pay_id"];
			$newSrd[$i][0]["pay_price"] = $mpId[$i]["pay_total"];
			$sqlMpD = "select * from c_srpayment where salesreceipt_id=" . $mpId[$i]["salesreceipt_id"] . " order by srpayment_id asc ";
			$mpDId = $this->getResult($sqlMpD);

			//echo $sqlMpD."<br>";
			for ($j = 0; $j < $mpDId["rows"]; $j++) {
				//echo "sr_id -> ".$srDId[$j]["srdetail_id"]."<br>";
				//$newSrd[$i][0]["sr_id"] = $mpDId[$j]["salesreceipt_id"];
				//$newSrd[$i][$j]["srd_id"] = $srDId[$j]["srdetail_id"];
				//$newSrd[$i][$j]["pd_id"] = $srDId[$j]["pd_id"];
				//$newSrd[$i][$j]["quantity"] = $srDId[$j]["qty"];
				//$newSrd[$i][$j]["unit_price"] = $srDId[$j]["unit_price"];
				//$newSrd[$i][$j]["plus_tax"] = $srDId[$j]["set_tax"];
				//$newSrd[$i][$j]["plus_sc"] = $srDId[$j]["set_sc"];
				$newSrd[$i][$j]["mpd_id"] = $mpDId[$j]["srpayment_id"];
				$newSrd[$i][$j]["paytype"] = $mpDId[$j]["pay_id"];
				$newSrd[$i][$j]["pay_price"] = $mpDId[$j]["pay_total"];
		...
		$srId = $this->getResult($sqlSr);
			if($srId){
				for ($i = 0; $i < $srId["rows"]; $i++) {
					//$newSrd[$i][0]["mpd_id"] = $srId[$i]["salesreceipt_id"];
					$newSrd[$i][0]["paytype"] = $srId[$i]["pay_id"];
					$newSrd[$i][0]["pay_price"] = $srId[$i]["sr_total"];
					//$sqlSrD = "select * from c_salesreceipt where salesreceipt_id=" . $srId[$i]["salesreceipt_id"] . " order by salesreceipt_id asc ";		
					//$srDId = $this->getResult($sqlSrD);

					//echo $sqlMpD."<br>";
					//for ($j = 0; $j < $srDId["rows"]; $j++) {
					//	$newSrd[$i][$j]["mpd_id"] = $srDId[$j]["salesreceipt_id"];
					//	$newSrd[$i][$j]["paytype"] = $srDId[$j]["pay_id"];
					//	$newSrd[$i][$j]["pay_price"] = $srDId[$j]["sr_total"];
					//}
				}
			}		
	...
	* @modified - add this function on 20 dec 2008
	*/
function deleteSaleReceiptData($srd, $bookid = 0, $pdsid = 0,$mpdcount) {
		...
		if ($srId) {
			...
			for ($i = 0; $i < $srId["rows"]; $i++) {
				...
				if (!$hasSr) {
						...
						for ($q = 0; $q < $srDId["rows"]; $q++) {
							...
							for ($si = 0; $si < count($srd); $si++) {
								for ($sj = 0; $sj < count($srd[$si]); $sj++) {
									if(!isset($srd[$si][$sj]["srd_id"])){$srd[$si][$sj]["srd_id"]="";}
					...
					$sqlMpD = "select srpayment_id from c_srpayment where salesreceipt_id=" . $srId[$i]["salesreceipt_id"];
					$mpDId = $this->getResult($sqlMpD);
					
					if ($mpDId) {
				
						for ($q = 0; $q < $mpDId["rows"]; $q++) {
							$hasMpdetail = false;

							for ($si = 0; $si < count($srd); $si++) {
								for ($sj = 0; $sj < $mpdcount[$si]; $sj++) {
									if(!isset($srd[$si][$sj]["mpd_id"])){$srd[$si][$sj]["mpd_id"]="";}
									if ($mpDId[$q]["srpayment_id"] == $srd[$si][$sj]["mpd_id"]) {
										$hasMpdetail = true;
									}
								}
							}
							if (!$hasMpdetail) {
								$deleteMpdetail[$countMpdetail] = $mpDId[$q]["srpayment_id"];
								$countMpdetail++;		
							}
						}
					}
			...
			for ($i = 0; $i < count($deleteMpdetail); $i++) {
				$this->updatelog_mp_detail($deleteMpdetail[$i], 0);
			}
			...
			if (count($deleteMpdetail) > 0) {
				$deleteMpdString = implode(",", $deleteMpdetail);
				$sql = "DELETE FROM c_srpayment WHERE srpayment_id in($deleteMpdString)";
				$this->setResult($sql);
			...
			for ($i = 0; $i < count($deleteSr); $i++) {
				...
				$sql = "select srpayment_id from c_srpayment where salesreceipt_id = " . $deleteSr[$i];
				$rs = $this->getResult($sql);
				for ($j = 0; $j < $rs["rows"]; $j++) {
					$this->updatelog_mp_detail($rs[$i]["srpayment_id"], 0);
				}
			}
			...
			if (count($deleteSr) > 0) {
				...
				$sql = "DELETE FROM c_srpayment WHERE salesreceipt_id in($deleteSrString)";
				$delete = $this->setResult($sql);
			}
	...
	function updatelog_sr_detail($srd_id = false, $active = 1, $debug = false) {
		...
		function updatelog_mp_detail($srd_id = false, $active = 1, $debug = false) {
		$chksql = "select * from c_srpayment " .
		"where srpayment_id = " . $srd_id;
		$logrs = $this->getResult($chksql);

		$lsrd_id = $logrs[0]["srpayment_id"];
		$lbook_id = $logrs[0]["book_id"];
		$lsr_id = $logrs[0]["salesreceipt_id"];
		$lpds_id = $logrs[0]["pds_id"];
		$lp_type = $logrs[0]["pay_id"];
		$lp_price = $logrs[0]["pay_total"];
		$ll_lu_user = $_SESSION["__user_id"];
		$ll_lu_ip = $_SERVER["REMOTE_ADDR"];
		$ll_datets = "now()";
		$chksql = "insert into log_c_srpayment(" .
		"srpayment_id,book_id,salesreceipt_id,pds_id,pay_id," .
		"pay_total," .
		"l_lu_user,l_lu_ip,l_lu_date,active) " .
		"value ('$lsrd_id','$lbook_id','$lsr_id','$lpds_id','$lp_type'," .
		"'$lp_price'," .
		"'$ll_lu_user','$ll_lu_ip',$ll_datets,'$active')";
		$logid = $this->setResult($chksql);
		return $logid;
	}
	 ...
	 */
	function addThList($thid, $bid, $now = false, $debug = false) {
	 ...
	 * @modified - add this function on 20 may 2009
	 */
	function getThList($cityid = false, $branchid=false, $page = false, $order = false, $sort = false, $debug = false) {
		$rows = $this->getShowpage(); // number of rows per page
		$start = $rows * $page - $rows; // starting from record $start
		$sql = "select bl_th_list.*,bl_branchinfo.branch_name,l_employee.emp_nickname as therapist_name,bl_th_list.ot,l_employee.emp_code " .
		...
		if ($branchid && $branchid!=1) {
			$sql .= "and bl_branchinfo.branch_id=$branchid ";
		}
		if ($order == "th_shift") {
	...
	*/
	 ...
	 */
	function chkBlockColor($status = false, $type=false) {
		...
		for ($i = 0; $i < count($status); $i++) {
			...
			if($type == 2 && !$status[$i]){
				$colormask[$i] = "#a0e3ff"; //online booking
			}
		}
		return $colormask;
	}
	 ...
	 */
	function chkBlockData($appointment = false, $room_id = false, $time_start = false, $time_end = false, $isEdit = false, $timeperiod = false, $chkRsView = false) {
		$ans[] = array ();
		$ans["data"] = "";
		$ans["popup"] = "";
		$ans["color"] = "";
		...
		for ($i = 0; $i < count($appointment["room_ids"]); $i++) {
			...
			if ($checkdata) {
				...
				if(!isset($appointment["therapist_names"][$i][$key])){$appointment["therapist_names"][$i][$key]="";}	
				...
				if (($time_start >= $appointment["start"][$i]) && ($time_start < $appointment["end"][$i][$key]) && $room_id == $appointment["room_ids"][$i][$key]) {
					...
					if ($appointment["start"][$i] >= $time_start&&$appointment["start"][$i] <= $time_end) {
						...
						if ($isEdit || $chkRsView) {
							...
							if ($appointment["bpds_id"][$i] > 0) {
								...
								$ans["data"].="<input onclick=\"EditRoomStatus(".$appointment["book_id"][$i].",".$room_id.",'A')\" type='button' id='smenu' name='smenu' value='A'>:";
								$ans["data"].="<input onclick=\"EditRoomStatus(".$appointment["book_id"][$i].",".$room_id.",'I')\" type='button' id='smenu' name='smenu' value='I'>:";
								$ans["data"].="<input onclick=\"EditRoomStatus(".$appointment["book_id"][$i].",".$room_id.",'F')\" type='button' id='smenu' name='smenu' value='F'>";								
							} else {
					...
					if (isset ($appointment["driver_names"][$i][0]) && isset ($appointment["driver_names"][$i][1])) {
						...
						$ans["color"] = $appointment["colormark"][$i][$key]; //online booking
					}
					$sql_chkpaid="select * from c_salesreceipt where book_id=".$appointment["book_id"][$i];
								$chkpaid=$this->getResult($sql_chkpaid);
								if($chkpaid && $appointment["start"][$i] == $time_start){
									$sta_paid=true;
									for($c=0;$c<$chkpaid["rows"];$c++){
										if($chkpaid[$c]["paid_confirm"]==0){
								...
								if($sta_paid){
								$ans["data"] .= "<br><b style=\"color:red;font-size:10px\">&nbsp;--Paid--</b>";
								}
					...
					if ($appointment["start"][$i] + $appointment["timeperiod"][$i] == $time_start) {
						...
						if ($appointment["therapist_names"][$i][$key] == "Maintenance Room") {
							...
							$ans["data"] = "<b style=\" font-size:11px; font-family:Tahoma; \">" . str_replace(" -- select --", " ", $appointment["cs_name"][$i]) . "</b>";
						}
					...
					if ($appointment["start"][$i] + 2*$appointment["timeperiod"][$i] == $time_start) {
						$ans["data"] = "";
						if ($appointment["therapist_names"][$i][$key] == "Maintenance Room") {
							$ans["data"] = " ";
						} else {
							$ans["data"] = "<b style=\" font-size:11px; font-family:Tahoma; \">" . str_replace(" -- select --", " ", $appointment["therapist_names"][$i][$key]) . "</b>";
						}

					}
					
					//IF >1.00 Booking 
					if($appointment["end"][$i][$key] - $appointment["start"][$i]<=2*$appointment["timeperiod"][$i]){
					  if($appointment["end"][$i][$key] - $appointment["start"][$i]==$appointment["timeperiod"][$i]){
						if ($appointment["therapist_names"][$i][$key] == "Maintenance Room") {
							$mhead = ($appointment["therapist_names"][$i][$key] == "Maintenance Room") ? "Room Maintenance" : $appointment["therapist_names"][$i][$key];
							//$ans["data"] = " ";
						} else {
						$ans["data"] .= "<br><b style=\" font-size:11px; font-family:Tahoma; \">" . str_replace(" -- select --", " ", $appointment["cs_name"][$i]) . "</b>";
						$ans["data"] .= "<br><b style=\" font-size:11px; font-family:Tahoma; \">" . str_replace(" -- select --", " ", $appointment["therapist_names"][$i][$key]) . "</b>";
						}
					  }elseif($appointment["start"][$i]!=$time_start){
					  	$ans["data"] .= "<br><b style=\" font-size:11px; font-family:Tahoma; \">" . str_replace(" -- select --", " ", $appointment["therapist_names"][$i][$key]) . "</b>";
					  }
					}
					...
					$memcode=array();
					$memcode[$i]=$this->getIdToText($appointment["book_id"][$i],"a_bookinginfo","a_member_code","book_id");
					$mpic=$this->getIdToText($memcode[$i],"m_membership","mpic","member_code");
					
					$pic="<img border='2' src=$customize_part/images/member/$mpic width='60px' height='60px'>";
					$msg = ($memcode[$i])?"$pic<br/>":"";
					...
					//Start for customer name in treatment information
					$rst_cs="select cs_name from d_indivi_info where book_id=".$appointment["book_id"][$i];
					$rst_cs.=" and room_id=".$room_id;
					$tre_cs=$this->getResult($rst_cs);
					for($t=0;$t<$tre_cs["rows"];$t++){
						if(!$tre_cs[$t]["cs_name"]){
							$tre_cs[$t]["cs_name"]="--select--";
						}
						$msg .= ($t+1).". ".$tre_cs[$t]["cs_name"]."<br>";
					}
					//End for customer name in treatment information
	 ...
	 * @modified - add this function on 24-Dec-2008 
	 */
	function getTherapistcount($appointment = false, $time_start = false, $time_end = false, $type=false, $date=false, $branchid=false) {
		$cnt = 0;
		$chk_id="";
	if($type=="th"){
		$list_book=array();
			...
			if(isset($appointment["hour_ids"][$i])){
				...
				for($j=0;$j<count($hour);$j++){
					if($appointment["start"][$i]<=$time_start&&$appointment["start"][$i]+6*($hour[$j]-1)>$time_start&&$appointment["bpds_id"][$i]!=0){
						array_push($list_book ,$appointment["book_id"][$i]);
					}
				}
			}
		}
		$list_book = array_unique($list_book);
		$list_book =implode(",",$list_book);
	//Check Therapist work in time
	/*	
		$time_start_name=$this->getIdToText($time_start,"p_timer","time_start","time_id");
		$time_end_name=$this->getIdToText($time_end,"p_timer","time_start","time_id");
		
		$sql_th="select da_mult_th.book_id,(select time_start from p_timer where time_id=da_mult_th.start_id) " .
				"as start_name ," .
				"(select time_start from p_timer where time_id=da_mult_th.end_id) " .
				"as end_name from da_mult_th";
		$th_cnt = $this->getResult($sql_th);
		for($k=0;$k<$th_cnt["rows"];$k++){
			if($th_cnt[$k]["start_name"]>=$time_start_name && $th_cnt[$k]["start_name"]<$time_end_name){
				$cnt++;
			}
			if($th_cnt[$k]["start_name"]<$time_start_name && $th_cnt[$k]["end_name"]>$time_start_name){
				$cnt++;
			}
		}
	*/
	...
	/*
	$sql_th="select da_mult_th.* from da_mult_th " .
			"where" .
			"(da_mult_th.start_id>=$time_start and da_mult_th.start_id<$time_end and start_id<>0 and end_id<>0 and (select b_branch_id from a_bookinginfo where book_id=da_mult_th.book_id)=$branchid and (select b_appt_date  from a_bookinginfo where book_id=da_mult_th.book_id)='$date' and (select b_set_cancel  from a_bookinginfo where book_id=da_mult_th.book_id)<>1) " .
			"or (da_mult_th.start_id<$time_start and da_mult_th.end_id>$time_start and start_id<>0 and end_id<>0 and (select b_branch_id from a_bookinginfo where book_id=da_mult_th.book_id)=$branchid and (select b_appt_date  from a_bookinginfo where book_id=da_mult_th.book_id)='$date' and (select b_set_cancel  from a_bookinginfo where book_id=da_mult_th.book_id)<>1) ";
	*/
	
	$sql_th="select da_mult_th.* from da_mult_th,a_bookinginfo where " .
			"(da_mult_th.start_id>=$time_start and da_mult_th.start_id<$time_end and start_id<>0 and end_id<>0 and a_bookinginfo.b_branch_id=$branchid and a_bookinginfo.b_appt_date='$date' and a_bookinginfo.b_set_cancel<>1 and a_bookinginfo.book_id=da_mult_th.book_id)" .
			"or (da_mult_th.start_id<$time_start and da_mult_th.end_id>$time_start and start_id<>0 and start_id<>0 and end_id<>0 and a_bookinginfo.b_branch_id=$branchid and a_bookinginfo.b_appt_date='$date' and a_bookinginfo.b_set_cancel<>1 and a_bookinginfo.book_id=da_mult_th.book_id)";
	...
	///////Set No TH color
	if($list_book){
	$sql_chk="select therapist_id from da_mult_th " .
			"where therapist_id in (0,1) " .
			"and book_id in ($list_book)";
	$chk_id = $this->getResult($sql_chk);
	}
	///////
	}
	...
	if($type=="cust"){
		...
		if ($cnt == 0) {
			$cnt = "&nbsp;";
			 ...
			 $app_time=$this->getIdToText($time_start,"p_timer","time_start","time_id");
		///////Set No TH color
			if($chk_id){
				//$cnt="<a href='javascript:;' onclick=openthQueue('&amp;type=&amp;time=$app_time&amp;branchid='+document.getElementById('branch_id').value+'&amp;date='+document.getElementById('hidden_date').value,'thinfo.php','thinfo');><font color=red>".$cnt."</font></a>";
				$cnt="<a href='javascript:;' onclick=openthQueue('&amp;time=$app_time&amp;branchid='+document.getElementById('branch_id').value+'&amp;date='+document.getElementById('hidden_date').value,'thinfo.php','thinfo');><font color=red>".$cnt."</font></a>";
			}else if($type=="th"){
				$cnt="<a href='javascript:;' onclick=openthQueue('&amp;time=$app_time&amp;branchid='+document.getElementById('branch_id').value+'&amp;date='+document.getElementById('hidden_date').value,'thinfo.php','thinfo');><font color=green>".$cnt."</font></a>";
			}
		//////
		echo $cnt;
	}
	...
	function saveCopyAppoiontment($book_id, $id, $tw, $date, $branch_id, $apptime, $city_change, $debug = false) { // $book_id - old booking id, $id - new copy booking id
		...
		$appointment["therapist_names_new"] = $rs[0]["th_names"];
		...
		for ($i = 0; $i < count($tw); $i++) {
		if(!isset($appointment["therapist_names_old"][$i])){$appointment["therapist_names_old"][$i]="";}
		if(!isset($appointment["hour_ids_old"][$i])){$appointment["hour_ids_old"][$i]="";}
		...
		$th_names1 = $appointment["therapist_names_new"];
		...
		",\"$qtyPeoples\",\"$th_names1\",\"$hourIds\",\"" . $appointment["accdt_name"] . "\",\"" . $appointment["t_times"] . "\"," .
	...
	*/
				...
				if ($chkStat) {
					...
					if(!$chk){
						if($tw[$i][0]["name"]==0){
							...
							$th[$j] .= "," . $this->getIdToText($tw[$i][0]["name"], "l_employee", "emp_nickname", "emp_id");
							$hId[$j] .= "," . $tw[$i]["tthour"];
						}
					}
	...
	function checkMemberbalance($membercode = false) {
		//table a_bookinginfo
