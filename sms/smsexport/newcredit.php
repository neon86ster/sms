...
$content.= 	"<tr>" .
 			...
 			"<td>".$today1."</td>";
 												if($rs_data[$ss]["book_id"]){
 													$sqlbook="select bl_branchinfo.branch_name from a_bookinginfo,bl_branchinfo where a_bookinginfo.book_id=".$rs_data[$ss]["book_id"]." " .
 															"and a_bookinginfo.b_branch_id=bl_branchinfo.branch_id";
 													 $br= mysql_query($sqlbook);
 													 
 													 //echo $sqlbook."<br>";
 										while($ii = mysql_fetch_array($br)){    
		 					 $rs_b[] = array( 'branch_name'=>$ii['branch_name']); 
		  					 }
 													 
				
							}
							if($rs_data[$ss]["pds_id"]){
								$sqlbook="select bl_branchinfo.branch_name from c_saleproduct,bl_branchinfo where c_saleproduct.pds_id=".$rs_data[$ss]["pds_id"]." " .
 ...
 $content.=	"<td>Branches:".$rs_b[$ss]["branch_name"]."</td>".
 			"<td></td>" .
 			"<td>1</td>" .
    ...
    $sql1 = "select c_salesreceipt.salesreceipt_id,a_bookinginfo.b_branch_id as branch_id " .
		...
		while($i = mysql_fetch_array($cc)){    
		 ...
		 for($i=0;$i<$rs_ptype["rows"];$i++){
		  			...
		  			$sql_data="select c_srpayment.*,l_paytype.pay_id,l_paytype.pay_name,pay_category_name,l_account.pay_account_name,l_template.pay_template_name,bl_branchinfo.branch_name,c_bpds_link.bpds_id as books_id,al_bookparty.bp_name  " .
